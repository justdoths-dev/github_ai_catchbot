...
def _build_logger(level: str) -> logging.Logger:
    ...
    return logging.getLogger("analysis-router")
    ...
    class SessionBackedService:
        async def handle_trigger_event(self, trigger_event_id: str):
            async with session_factory.begin() as session:
                ...
                service = AnalysisRouterService(config, repository=repository, logger=logger)
                return await service.handle_trigger_event(trigger_event_id)

    worker = AnalysisRouterWorker(config, consumer=consumer, service=SessionBackedService(), logger=logger)
    try:
        await worker.run_forever()
    except asyncio.CancelledError:
        logger.info("analysis_router_cancelled", extra={"service": "analysis-router", "event": "cancelled"})
        return 0
    finally:
