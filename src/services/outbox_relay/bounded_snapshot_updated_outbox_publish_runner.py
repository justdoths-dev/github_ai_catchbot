...
SCHEMA_VERSION = "bounded_snapshot_updated_outbox_publish_v1"
RUNNER_NAME = "bounded_snapshot_updated_outbox_publish_runner"
MODE = "snapshot_updated_outbox_one_shot_publish"
...
class BoundedSnapshotUpdatedOutboxPublishConfig:
...
@dataclass(frozen=True, slots=True)
class BoundedSnapshotUpdatedPublishRuntimeConfig:
    database_url: str
    redis_url: str
...
class BoundedSnapshotUpdatedOutboxPublishState:
...
class BoundedSnapshotUpdatedOutboxPublishError(RuntimeError):
...
class BoundedSnapshotUpdatedOutboxRepository(Protocol):
...
@dataclass(frozen=True, slots=True)
class BoundedSnapshotUpdatedRepositoryHandle:
    repository: BoundedSnapshotUpdatedOutboxRepository
    close: Callable[[bool], Awaitable[None]]


@dataclass(frozen=True, slots=True)
class BoundedSnapshotUpdatedRedisPublisherHandle:
    publisher: RedisPublisher
    close: Callable[[], Awaitable[None]]


class BoundedSnapshotUpdatedRepositoryBuilder(Protocol):
    async def __call__(
        self,
        runtime_config: BoundedSnapshotUpdatedPublishRuntimeConfig,
        state: BoundedSnapshotUpdatedOutboxPublishState,
        logger: logging.Logger,
    ) -> BoundedSnapshotUpdatedRepositoryHandle: ...


class BoundedSnapshotUpdatedRedisPublisherBuilder(Protocol):
    async def __call__(
        self,
        runtime_config: BoundedSnapshotUpdatedPublishRuntimeConfig,
        state: BoundedSnapshotUpdatedOutboxPublishState,
...
@dataclass(frozen=True, slots=True)
class BoundedSnapshotUpdatedOutboxPublishResult:
    status: str
    ok: bool
    error_code: str | None
    error_class: str | None
    config: BoundedSnapshotUpdatedOutboxPublishConfig
    state: BoundedSnapshotUpdatedOutboxPublishState = field(
        default_factory=BoundedSnapshotUpdatedOutboxPublishState
    )
    ...
    selected_payload_provider: str | None = None
    selected_payload_provider_route: str | None = None
    payload_has_artifact_id: bool = False
    payload_has_snapshot_id: bool = False
    payload_has_provider: bool = False
    payload_has_provider_route: bool = False
    payload_has_snapshot_type: bool = False
    payload_has_status: bool = False
    payload_has_content_anchor: bool = False
    ...
    def to_sanitized_dict(self) -> dict[str, Any]:
        return {
            ...
            "selected_payload_provider": self.selected_payload_provider,
            "selected_payload_provider_route": self.selected_payload_provider_route,
            ...
            "payload_has_artifact_id": self.payload_has_artifact_id,
            "payload_has_snapshot_id": self.payload_has_snapshot_id,
            "payload_has_provider": self.payload_has_provider,
            "payload_has_provider_route": self.payload_has_provider_route,
            "payload_has_snapshot_type": self.payload_has_snapshot_type,
            "payload_has_status": self.payload_has_status,
            "payload_has_content_anchor": self.payload_has_content_anchor,
            "event_outbox_marked_published": self.event_outbox_marked_published,
            "redactions_applied": [
                "full_event_id_omitted",
                "full_artifact_id_omitted",
                "full_snapshot_id_omitted",
                "idempotency_key_omitted",
                "payload_json_omitted",
                "snapshot_metadata_omitted",
                "final_url_omitted",
                "article_text_omitted",
                "title_description_omitted",
...
class SqlAlchemyBoundedSnapshotUpdatedOutboxRepository:
        ...
        *,
        ...
        if event_id is not None:
            ...
            statement = _SELECT_TARGET_EVENT + """
                ...
                LIMIT :limit
                """
            params = {"event_type": EVENT_TYPE, "artifact_id": str(artifact_id), "limit": limit}
            ...
            raise BoundedSnapshotUpdatedOutboxPublishError("target_missing")
...
def load_bounded_snapshot_updated_publish_runtime_config(
    ...
    if not database_url:
        raise BoundedSnapshotUpdatedOutboxPublishError("database_url_missing")
    if not redis_url:
        raise BoundedSnapshotUpdatedOutboxPublishError("redis_url_missing")
    ...
    try:
        ...
        raise BoundedSnapshotUpdatedOutboxPublishError("runtime_config_error") from exc
    if xadd_maxlen is not None and xadd_maxlen <= 0:
        raise BoundedSnapshotUpdatedOutboxPublishError("runtime_config_error")
    return BoundedSnapshotUpdatedPublishRuntimeConfig(
...
async def build_default_bounded_snapshot_updated_repository(
    runtime_config: BoundedSnapshotUpdatedPublishRuntimeConfig,
    state: BoundedSnapshotUpdatedOutboxPublishState,
    logger: logging.Logger,
) -> BoundedSnapshotUpdatedRepositoryHandle:
    ...
    repository = SqlAlchemyBoundedSnapshotUpdatedOutboxRepository(session)
    ...
    return BoundedSnapshotUpdatedRepositoryHandle(repository=repository, close=close)


async def build_default_bounded_snapshot_updated_redis_publisher(
    runtime_config: BoundedSnapshotUpdatedPublishRuntimeConfig,
    state: BoundedSnapshotUpdatedOutboxPublishState,
    logger: logging.Logger,
) -> BoundedSnapshotUpdatedRedisPublisherHandle:
    ...
    return BoundedSnapshotUpdatedRedisPublisherHandle(publisher=publisher, close=close)


async def run_bounded_snapshot_updated_outbox_publish(
    config: BoundedSnapshotUpdatedOutboxPublishConfig,
    *,
    runtime_config_loader: Callable[[], BoundedSnapshotUpdatedPublishRuntimeConfig] = (
        load_bounded_snapshot_updated_publish_runtime_config
    ),
    ...
    except BoundedSnapshotUpdatedOutboxPublishError as exc:
    ...
    repository_handle: BoundedSnapshotUpdatedRepositoryHandle | None = None
    publisher_handle: BoundedSnapshotUpdatedRedisPublisherHandle | None = None
    commit_repository = False
    result: BoundedSnapshotUpdatedOutboxPublishResult | None = None
    try:
        repository_handle = await (repository_builder or build_default_bounded_snapshot_updated_repository)(
            runtime_config,
            state,
            effective_logger,
        )
        ...
        provider = _payload_string(row.payload_json, "provider")
        provider_route = _payload_string(row.payload_json, "provider_route")
        snapshot_id = _payload_uuid(row.payload_json, "snapshot_id")
        ...
        if row.event_type != EVENT_TYPE or row.aggregate_type != ROOT_OBJECT_TYPE:
            result = _result(
                "blocked",
                "snapshot_updated_event_contract_mismatch",
                config=config,
                state=state,
                selected_event=row,
                events_seen=events_seen,
        ...
        if not all(payload_flags.values()):
            ...
            raise _PublishResultReady
        if provider is None and provider_route is None:
            result = _result(
                "blocked",
                "provider_missing",
                config=config,
            ...
            raise _PublishResultReady
        if not _payload_artifact_id_matches(row):
            result = _result(
                "blocked",
        ...
        publisher_handle = await (redis_publisher_builder or build_default_bounded_snapshot_updated_redis_publisher)(
            runtime_config,
            state,
            effective_logger,
        )
...
def run_bounded_snapshot_updated_outbox_publish_sync(
    config: BoundedSnapshotUpdatedOutboxPublishConfig,
    *,
    runtime_config_loader: Callable[[], BoundedSnapshotUpdatedPublishRuntimeConfig] = (
        load_bounded_snapshot_updated_publish_runtime_config
...
def _result(
    ...
    *,
    ...
    selected_artifact_id = (
        ...
        else config.artifact_id
    )
    return BoundedSnapshotUpdatedOutboxPublishResult(
        status=status,
        ...
        target_artifact_id_suffix=_optional_id_suffix(selected_artifact_id),
        ...
        selected_payload_provider=provider,
        selected_payload_provider_route=provider_route,
        payload_has_artifact_id=flags.get("artifact_id", False),
        payload_has_snapshot_id=flags.get("snapshot_id", False),
        payload_has_provider=provider is not None,
        payload_has_provider_route=provider_route is not None,
        payload_has_snapshot_type=flags.get("snapshot_type", False),
        payload_has_status=flags.get("status", False),
        payload_has_content_anchor=flags.get("content_anchor", False),
        event_outbox_marked_published=event_outbox_marked_published,
    )
...
def _payload_artifact_id_matches(row: OutboxEventRow) -> bool:
    try:
        return UUID(str(row.payload_json.get("artifact_id"))) == row.aggregate_id
    except (TypeError, ValueError):
        return False
...
def _selector_count(config: BoundedSnapshotUpdatedOutboxPublishConfig) -> int:
    return sum(value is not None for value in (config.event_id, config.artifact_id, config.event_suffix))


def _selector_type(config: BoundedSnapshotUpdatedOutboxPublishConfig) -> str | None:
...
__all__ = [
    ...
    "BoundedSnapshotUpdatedOutboxPublishError",
    "BoundedSnapshotUpdatedOutboxPublishResult",
    "BoundedSnapshotUpdatedPublishRuntimeConfig",
    ...
    "SCHEMA_VERSION",
    "STAGE_NAME",
    "SqlAlchemyBoundedSnapshotUpdatedOutboxRepository",
    "argument_error_report",
    "build_default_bounded_snapshot_updated_redis_publisher",
    "build_default_bounded_snapshot_updated_repository",
    "load_bounded_snapshot_updated_publish_runtime_config",
