...
SCHEMA_VERSION = "bounded_artifact_enrich_outbox_publish_v1"
RUNNER_NAME = "bounded_artifact_enrich_outbox_publish_runner"
MODE = "artifact_enrich_outbox_one_shot_publish"
EVENT_TYPE = "artifact.enrich.requested.v1"
ROOT_OBJECT_TYPE = "artifact"
DEFAULT_XADD_MAXLEN = 10000
DEFAULT_MAX_EVENTS = 1
HARD_MAX_EVENTS = 1
ALLOWED_PROVIDER_ROUTES = frozenset({"github", "x", "web"})
...
class BoundedArtifactEnrichOutboxPublishConfig:
...
@dataclass(frozen=True, slots=True)
class BoundedArtifactEnrichPublishRuntimeConfig:
    database_url: str
    redis_url: str
...
class BoundedArtifactEnrichOutboxPublishState:
...
class BoundedArtifactEnrichOutboxPublishError(RuntimeError):
...
class BoundedArtifactEnrichOutboxRepository(Protocol):
...
@dataclass(frozen=True, slots=True)
class BoundedArtifactEnrichRepositoryHandle:
    repository: BoundedArtifactEnrichOutboxRepository
    close: Callable[[bool], Awaitable[None]]


@dataclass(frozen=True, slots=True)
class BoundedArtifactEnrichRedisPublisherHandle:
    publisher: RedisPublisher
    close: Callable[[], Awaitable[None]]


class BoundedArtifactEnrichRepositoryBuilder(Protocol):
    async def __call__(
        self,
        runtime_config: BoundedArtifactEnrichPublishRuntimeConfig,
        state: BoundedArtifactEnrichOutboxPublishState,
        logger: logging.Logger,
    ) -> BoundedArtifactEnrichRepositoryHandle: ...


class BoundedArtifactEnrichRedisPublisherBuilder(Protocol):
    async def __call__(
        self,
        runtime_config: BoundedArtifactEnrichPublishRuntimeConfig,
        state: BoundedArtifactEnrichOutboxPublishState,
...
@dataclass(frozen=True, slots=True)
class BoundedArtifactEnrichOutboxPublishResult:
    status: str
    ok: bool
    error_code: str | None
    error_class: str | None
    config: BoundedArtifactEnrichOutboxPublishConfig
    state: BoundedArtifactEnrichOutboxPublishState = field(
        default_factory=BoundedArtifactEnrichOutboxPublishState
    )
    ...
    payload_has_artifact_type: bool = False
    payload_has_provider_route: bool = False
    payload_has_refresh_mode: bool = False
    payload_has_depth_budget: bool = False
    ...
    def to_sanitized_dict(self) -> dict[str, Any]:
        return {
            ...
            "selected_provider_route": self.selected_provider_route,
            ...
            "payload_has_artifact_type": self.payload_has_artifact_type,
            "payload_has_provider_route": self.payload_has_provider_route,
            "payload_has_refresh_mode": self.payload_has_refresh_mode,
            "payload_has_depth_budget": self.payload_has_depth_budget,
...
class SqlAlchemyBoundedArtifactEnrichOutboxRepository:
    ...
    async def fetch_target_events(
        ...
        *,
        ...
        if event_id is not None:
            ...
            raise BoundedArtifactEnrichOutboxPublishError("target_missing")
...
def load_bounded_artifact_enrich_publish_runtime_config(
    ...
    if not database_url:
        raise BoundedArtifactEnrichOutboxPublishError("database_url_missing")
    if not redis_url:
        raise BoundedArtifactEnrichOutboxPublishError("redis_url_missing")
    ...
    try:
        ...
        raise BoundedArtifactEnrichOutboxPublishError("runtime_config_error") from exc
    if xadd_maxlen is not None and xadd_maxlen <= 0:
        raise BoundedArtifactEnrichOutboxPublishError("runtime_config_error")
    return BoundedArtifactEnrichPublishRuntimeConfig(
...
async def build_default_bounded_artifact_enrich_repository(
    runtime_config: BoundedArtifactEnrichPublishRuntimeConfig,
    state: BoundedArtifactEnrichOutboxPublishState,
    logger: logging.Logger,
) -> BoundedArtifactEnrichRepositoryHandle:
    ...
    repository = SqlAlchemyBoundedArtifactEnrichOutboxRepository(session)
    ...
    return BoundedArtifactEnrichRepositoryHandle(repository=repository, close=close)


async def build_default_bounded_artifact_enrich_redis_publisher(
    runtime_config: BoundedArtifactEnrichPublishRuntimeConfig,
    state: BoundedArtifactEnrichOutboxPublishState,
    logger: logging.Logger,
) -> BoundedArtifactEnrichRedisPublisherHandle:
    ...
    return BoundedArtifactEnrichRedisPublisherHandle(publisher=publisher, close=close)


async def run_bounded_artifact_enrich_outbox_publish(
    config: BoundedArtifactEnrichOutboxPublishConfig,
    *,
    runtime_config_loader: Callable[[], BoundedArtifactEnrichPublishRuntimeConfig] = (
        load_bounded_artifact_enrich_publish_runtime_config
    ),
    repository_builder: BoundedArtifactEnrichRepositoryBuilder | None = None,
    redis_publisher_builder: BoundedArtifactEnrichRedisPublisherBuilder | None = None,
    route_resolver: OutboxRouteResolver | None = None,
    clock: Callable[[], datetime] | None = None,
    logger: logging.Logger | None = None,
) -> BoundedArtifactEnrichOutboxPublishResult:
    state = BoundedArtifactEnrichOutboxPublishState()
    ...
    except BoundedArtifactEnrichOutboxPublishError as exc:
    ...
    repository_handle: BoundedArtifactEnrichRepositoryHandle | None = None
    publisher_handle: BoundedArtifactEnrichRedisPublisherHandle | None = None
    commit_repository = False
    result: BoundedArtifactEnrichOutboxPublishResult | None = None
    try:
        repository_handle = await (repository_builder or build_default_bounded_artifact_enrich_repository)(
            runtime_config,
            state,
            effective_logger,
        )
        ...
        if row.event_type != EVENT_TYPE or row.aggregate_type != ROOT_OBJECT_TYPE:
            result = _result(
                "blocked",
                "artifact_enrich_event_contract_mismatch",
                config=config,
                state=state,
                selected_event=row,
                events_seen=events_seen,
        ...
        if not all(payload_flags.values()):
            ...
            raise _PublishResultReady
        if provider_route not in ALLOWED_PROVIDER_ROUTES:
            result = _result(
                "blocked",
                "provider_route_not_allowed",
                config=config,
                state=state,
                selected_event=row,
                events_seen=events_seen,
        ...
        publisher_handle = await (redis_publisher_builder or build_default_bounded_artifact_enrich_redis_publisher)(
            runtime_config,
            state,
            effective_logger,
        )
...
def run_bounded_artifact_enrich_outbox_publish_sync(
    config: BoundedArtifactEnrichOutboxPublishConfig,
    *,
    runtime_config_loader: Callable[[], BoundedArtifactEnrichPublishRuntimeConfig] = (
        load_bounded_artifact_enrich_publish_runtime_config
...
def _result(
    ...
    *,
    ...
    return BoundedArtifactEnrichOutboxPublishResult(
        status=status,
        ...
        selected_provider_route=provider_route,
        payload_has_candidate_group_id=flags.get("candidate_group_id", False),
        payload_has_artifact_id=flags.get("artifact_id", False),
        payload_has_artifact_type=flags.get("artifact_type", False),
        payload_has_provider_route=flags.get("provider_route", False),
        payload_has_refresh_mode=flags.get("refresh_mode", False),
        payload_has_depth_budget=flags.get("depth_budget", False),
        event_outbox_marked_published=event_outbox_marked_published,
    )
...
def _selector_count(config: BoundedArtifactEnrichOutboxPublishConfig) -> int:
    return sum(value is not None for value in (config.event_id, config.artifact_id, config.event_suffix))


def _selector_type(config: BoundedArtifactEnrichOutboxPublishConfig) -> str | None:
...
__all__ = [
    ...
    "BoundedArtifactEnrichOutboxPublishError",
    "BoundedArtifactEnrichOutboxPublishResult",
    "BoundedArtifactEnrichPublishRuntimeConfig",
    "BoundedArtifactEnrichRedisPublisherBuilder",
    "BoundedArtifactEnrichRedisPublisherHandle",
    "BoundedArtifactEnrichRepositoryBuilder",
    "BoundedArtifactEnrichRepositoryHandle",
    "DEFAULT_MAX_EVENTS",
    "EVENT_TYPE",
    "HARD_MAX_EVENTS",
    ...
    "RUNNER_NAME",
    "SCHEMA_VERSION",
    "SqlAlchemyBoundedArtifactEnrichOutboxRepository",
    "argument_error_report",
    "build_default_bounded_artifact_enrich_redis_publisher",
    "build_default_bounded_artifact_enrich_repository",
    "load_bounded_artifact_enrich_publish_runtime_config",
    "render_sanitized_json",
    "run_bounded_artifact_enrich_outbox_publish",
    "run_bounded_artifact_enrich_outbox_publish_sync",
]
