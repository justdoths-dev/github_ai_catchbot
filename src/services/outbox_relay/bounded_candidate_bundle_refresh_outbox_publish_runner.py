...
SCHEMA_VERSION = "bounded_candidate_bundle_refresh_outbox_publish_v1"
RUNNER_NAME = "bounded_candidate_bundle_refresh_outbox_publish_runner"
MODE = "candidate_bundle_refresh_outbox_one_shot_publish"
...
OPTIONAL_PAYLOAD_FIELDS = ("source_version_no", "bundle_id")
PAYLOAD_REPORT_FIELDS = (*REQUIRED_PAYLOAD_FIELDS, *OPTIONAL_PAYLOAD_FIELDS)


@dataclass(frozen=True, slots=True)
class BoundedCandidateBundleRefreshOutboxPublishConfig:
...
@dataclass(frozen=True, slots=True)
class BoundedCandidateBundleRefreshPublishRuntimeConfig:
    database_url: str
    redis_url: str
...
class BoundedCandidateBundleRefreshOutboxPublishState:
...
class BoundedCandidateBundleRefreshOutboxPublishError(RuntimeError):
...
class BoundedCandidateBundleRefreshOutboxRepository(Protocol):
...
@dataclass(frozen=True, slots=True)
class BoundedCandidateBundleRefreshRepositoryHandle:
    repository: BoundedCandidateBundleRefreshOutboxRepository
    close: Callable[[bool], Awaitable[None]]


@dataclass(frozen=True, slots=True)
class BoundedCandidateBundleRefreshRedisPublisherHandle:
    publisher: RedisPublisher
    close: Callable[[], Awaitable[None]]


class BoundedCandidateBundleRefreshRepositoryBuilder(Protocol):
    async def __call__(
        self,
...
class BoundedCandidateBundleRefreshRedisPublisherBuilder(Protocol):
    async def __call__(
        self,
        runtime_config: BoundedCandidateBundleRefreshPublishRuntimeConfig,
        state: BoundedCandidateBundleRefreshOutboxPublishState,
...
@dataclass(frozen=True, slots=True)
class BoundedCandidateBundleRefreshOutboxPublishResult:
    status: str
    ok: bool
    error_code: str | None
    error_class: str | None
    config: BoundedCandidateBundleRefreshOutboxPublishConfig
    state: BoundedCandidateBundleRefreshOutboxPublishState = field(
        default_factory=BoundedCandidateBundleRefreshOutboxPublishState
    )
    selector_type: str | None = None
    target_event_id_suffix: str | None = None
    target_candidate_group_id_suffix: str | None = None
    events_seen: int = 0
    ...
    payload_has_trigger_kind: bool = False
    payload_has_trigger_object_type: bool = False
    payload_has_trigger_object_id: bool = False
    payload_has_refresh_reason: bool = False
    payload_has_source_version_no: bool = False
    ...
    def to_sanitized_dict(self) -> dict[str, Any]:
        return {
            ...
            "target_candidate_group_id_suffix": self.target_candidate_group_id_suffix,
            ...
            "payload_has_trigger_kind": self.payload_has_trigger_kind,
            "payload_has_trigger_object_type": self.payload_has_trigger_object_type,
            "payload_has_trigger_object_id": self.payload_has_trigger_object_id,
            "payload_has_refresh_reason": self.payload_has_refresh_reason,
            "payload_has_source_version_no": self.payload_has_source_version_no,
...
class SqlAlchemyBoundedCandidateBundleRefreshOutboxRepository:
    ...
    async def fetch_target_events(
        ...
        *,
        ...
        if event_id is not None:
        ...
        elif candidate_group_id is not None:
            statement = _SELECT_TARGET_EVENT + """
            ...
            raise BoundedCandidateBundleRefreshOutboxPublishError("target_missing")
...
def load_bounded_candidate_bundle_refresh_publish_runtime_config(
    ...
    if not database_url:
        raise BoundedCandidateBundleRefreshOutboxPublishError("database_url_missing")
    if not redis_url:
        raise BoundedCandidateBundleRefreshOutboxPublishError("redis_url_missing")
    ...
    try:
        ...
        raise BoundedCandidateBundleRefreshOutboxPublishError("runtime_config_error") from exc
    if xadd_maxlen is not None and xadd_maxlen <= 0:
        raise BoundedCandidateBundleRefreshOutboxPublishError("runtime_config_error")
    return BoundedCandidateBundleRefreshPublishRuntimeConfig(
...
async def build_default_bounded_candidate_bundle_refresh_repository(
    runtime_config: BoundedCandidateBundleRefreshPublishRuntimeConfig,
    state: BoundedCandidateBundleRefreshOutboxPublishState,
    logger: logging.Logger,
) -> BoundedCandidateBundleRefreshRepositoryHandle:
    ...
    repository = SqlAlchemyBoundedCandidateBundleRefreshOutboxRepository(session)
    ...
    return BoundedCandidateBundleRefreshRepositoryHandle(repository=repository, close=close)


async def build_default_bounded_candidate_bundle_refresh_redis_publisher(
    runtime_config: BoundedCandidateBundleRefreshPublishRuntimeConfig,
    state: BoundedCandidateBundleRefreshOutboxPublishState,
    logger: logging.Logger,
) -> BoundedCandidateBundleRefreshRedisPublisherHandle:
    ...
    return BoundedCandidateBundleRefreshRedisPublisherHandle(publisher=publisher, close=close)


async def run_bounded_candidate_bundle_refresh_outbox_publish(
    config: BoundedCandidateBundleRefreshOutboxPublishConfig,
    *,
    ...
    repository_builder: BoundedCandidateBundleRefreshRepositoryBuilder | None = None,
    redis_publisher_builder: BoundedCandidateBundleRefreshRedisPublisherBuilder | None = None,
    ...
    repository_handle: BoundedCandidateBundleRefreshRepositoryHandle | None = None
    publisher_handle: BoundedCandidateBundleRefreshRedisPublisherHandle | None = None
    commit_repository = False
    result: BoundedCandidateBundleRefreshOutboxPublishResult | None = None
    try:
        repository_handle = await (repository_builder or build_default_bounded_candidate_bundle_refresh_repository)(
            runtime_config,
            state,
            effective_logger,
        )
        ...
        rows = await repository.fetch_target_events(
            ...
            candidate_group_id=config.candidate_group_id,
        ...
        if row.event_type != EVENT_TYPE or row.aggregate_type != ROOT_OBJECT_TYPE:
            result = _result(
                "blocked",
                "candidate_bundle_refresh_event_contract_mismatch",
                config=config,
                state=state,
                selected_event=row,
                events_seen=events_seen,
        ...
        if not all(payload_flags.get(field_name, False) for field_name in REQUIRED_PAYLOAD_FIELDS):
            ...
            raise _PublishResultReady
        if not _payload_candidate_group_id_matches(row):
            result = _result(
                "blocked",
        ...
        publisher_handle = await (
            redis_publisher_builder or build_default_bounded_candidate_bundle_refresh_redis_publisher
        )(
            runtime_config,
            state,
            effective_logger,
...
def run_bounded_candidate_bundle_refresh_outbox_publish_sync(
    config: BoundedCandidateBundleRefreshOutboxPublishConfig,
    *,
    runtime_config_loader: Callable[[], BoundedCandidateBundleRefreshPublishRuntimeConfig] = (
        load_bounded_candidate_bundle_refresh_publish_runtime_config
...
def _result(
    ...
    *,
    ...
    selected_candidate_group_id = (
        ...
        else config.candidate_group_id
    )
    return BoundedCandidateBundleRefreshOutboxPublishResult(
        ...
        target_candidate_group_id_suffix=_optional_id_suffix(selected_candidate_group_id),
        events_seen=events_seen,
        ...
        payload_has_trigger_kind=flags.get("trigger_kind", False),
        payload_has_trigger_object_type=flags.get("trigger_object_type", False),
        payload_has_trigger_object_id=flags.get("trigger_object_id", False),
        payload_has_refresh_reason=flags.get("refresh_reason", False),
        payload_has_source_version_no=flags.get("source_version_no", False),
...
def _selector_count(config: BoundedCandidateBundleRefreshOutboxPublishConfig) -> int:
    return sum(value is not None for value in (config.event_id, config.candidate_group_id, config.event_suffix))


def _selector_type(config: BoundedCandidateBundleRefreshOutboxPublishConfig) -> str | None:
    if config.event_id is not None:
        return "event_id"
    if config.candidate_group_id is not None:
...
__all__ = [
    ...
    "BoundedCandidateBundleRefreshOutboxPublishResult",
    "BoundedCandidateBundleRefreshPublishRuntimeConfig",
    "BoundedCandidateBundleRefreshRedisPublisherBuilder",
    "BoundedCandidateBundleRefreshRedisPublisherHandle",
    "BoundedCandidateBundleRefreshRepositoryBuilder",
    "BoundedCandidateBundleRefreshRepositoryHandle",
    "DEFAULT_MAX_EVENTS",
    "EVENT_TYPE",
    "HARD_MAX_EVENTS",
    ...
    "build_default_bounded_candidate_bundle_refresh_redis_publisher",
    "build_default_bounded_candidate_bundle_refresh_repository",
    "load_bounded_candidate_bundle_refresh_publish_runtime_config",
    "render_sanitized_json",
    "run_bounded_candidate_bundle_refresh_outbox_publish",
    "run_bounded_candidate_bundle_refresh_outbox_publish_sync",
]
