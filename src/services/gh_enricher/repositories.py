...
class GhEnricherRepository:
    ...
    async def insert_snapshot(self, *, artifact_id: UUID, provider: str, plan: SnapshotWritePlan) -> UUID:
        result = await self._session.execute(
            sa.text(
                """
    ...
    async def insert_github_repo_child(self, *, snapshot_id: UUID, repo: GitHubRepoProjection) -> None:
        await self._session.execute(
            sa.text(
                """
            ...
            _repo_params(snapshot_id, repo),
        )

    async def insert_github_file_sample(self, *, snapshot_id: UUID, sample: GitHubFileSample) -> None:
        await self._session.execute(
            sa.text(
                """
