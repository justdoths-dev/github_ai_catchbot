...
class AnalysisValidatorWorker:
    def __init__(
        ...
        *,
        consumer: RedisStreamConsumer | RedisStreamConsumerProtocol,
    ...
    async def _process_message(self, message: StreamMessage) -> None:
        ...
        if not trigger_event_id:
            self._logger.error(
                "analysis_validator_stream_missing_trigger_event_id",
                extra={
                    "service": "analysis-validator",
                    "event": "analysis_validator_stream_missing_trigger_event_id",
