...
def _build_logger(level: str) -> logging.Logger:
    ...
    return logging.getLogger("analysis-validator")
    ...
    class SessionBackedService:
        async def handle_trigger_event(self, trigger_event_id: str):
            async with session_factory.begin() as session:
                ...
                service = AnalysisValidatorService(config, repository=repository, logger=logger)
                return await service.handle_trigger_event(trigger_event_id)

    worker = AnalysisValidatorWorker(config, consumer=consumer, service=SessionBackedService(), logger=logger)  # type: ignore[arg-type]
    try:
        await worker.run_forever()
    except asyncio.CancelledError:
        logger.info("analysis_validator_cancelled", extra={"service": "analysis-validator", "event": "cancelled"})
        return 0
    finally:
