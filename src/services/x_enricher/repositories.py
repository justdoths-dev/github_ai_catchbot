...
class XEnricherRepository:
    ...
    async def insert_snapshot(self, *, artifact_id: UUID, draft: XPostSnapshotDraft) -> UUID:
        result = await self._session.execute(
            sa.text(
                """
    ...
    async def upsert_x_post_child(self, *, snapshot_id: UUID, draft: XPostSnapshotDraft) -> None:
        await self._session.execute(
            sa.text(
                """
