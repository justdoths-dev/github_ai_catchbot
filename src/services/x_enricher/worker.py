...
class XEnricherWorker:
    ...
    async def run_forever(self) -> None:
        ...
        self._logger.info(
            "x_enricher_worker_started",
            extra={
                "service": "x-enricher",
                "event": "x_enricher_worker_started",
    ...
    async def _process_message(self, message: StreamMessage) -> None:
        ...
        if not trigger_event_id:
            self._logger.error(
                "x_enricher_stream_missing_trigger_event_id",
                extra={
                    "service": "x-enricher",
                    "event": "x_enricher_stream_missing_trigger_event_id",
        ...
        if job is None:
            self._logger.warning(
                "x_enricher_missing_outbox_job",
                extra={
                    "service": "x-enricher",
                    "event": "x_enricher_missing_outbox_job",
