...
def test_maintenance_cli_invocation_documents_one_shot_compose_and_confirm() -> None:
    ...
    assert "python /path/to/github_ai_catchbot/src/services/maintenance/worker_bootstrap.py" in text
    assert "python -m src.services.maintenance.worker_bootstrap" not in text
