...
SCRIPT = (
    ...
    / "scripts"
    / "ops"
    / "dedicated_vps_telegram_runtime_secret_placement_update_package_check.py"
)
RUNBOOK = (
    ROOT
    / "ops"
...
def _module():
    from scripts.ops import (
        dedicated_vps_telegram_runtime_secret_placement_update_package_check as module,
    )
...
def test_checker_returns_pass_for_committed_package() -> None:
    ...
    assert (
        report["report_type"]
        == "dedicated_vps_telegram_runtime_secret_placement_update_package_check_v1"
    )
...
def test_required_runbook_sections_are_checked(tmp_path: Path) -> None:
    text = _valid_runbook_text().replace("## Safe edit procedure", "## Edit procedure", 1)
...
def test_required_key_names_are_checked(tmp_path: Path) -> None:
    ...
    assert any(
        "TELEGRAM_API_ID" in failure.get("missing_keys", [])
...
def test_unsafe_command_patterns_are_rejected(tmp_path: Path) -> None:
    unsafe_snippets = {
        ...
        "echo_secret_append": "echo TELEGRAM_BOT_TOKEN=secret >> /etc/github-ai-catchbot/runtime.env",
...
def test_real_looking_secret_url_invite_and_ip_patterns_are_rejected(tmp_path: Path) -> None:
...
def test_tests_do_not_reference_runtime_env_file_for_reads() -> None:
    text = Path(__file__).read_text(encoding="utf-8")
    runtime_env_literal_single = "Path('/etc/github-ai-catchbot/" + "runtime.env')"
    runtime_env_literal_double = 'Path("/etc/github-ai-catchbot/' + 'runtime.env")'

    assert runtime_env_literal_single not in text
    assert runtime_env_literal_double not in text
