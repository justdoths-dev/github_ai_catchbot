...
SCRIPT = (
    ...
    / "scripts"
    / "ops"
    / "dedicated_vps_joined_channel_collector_startup_readiness_preflight.py"
)

FAKE_DATABASE_URL = (
    "postgresql+psycopg://github_ai_catchbot_app:"
    "unit-db-password-for-joined-channel-preflight@127.0.0.1:5432/github_ai_catchbot"
)
FAKE_REDIS_URL = (
    "redis://:unit-redis-secret-joined-channel-preflight@127.0.0.1:6379/0"
)
FAKE_DATABASE_PASSWORD = "unit-db-password-for-joined-channel-preflight"
FAKE_TELEGRAM_SECRET = "unit-telegram-api-hash-joined-channel-preflight"
RAW_CHAT_ID = 9876543210123
RAW_SOURCE_VALUE = "SensitiveJoinedChannel"
RAW_USERNAME = "SensitiveJoinedUsername"
RAW_TITLE = "Sensitive Joined Channel Title"
RAW_INVITE_LINK = "https://t.me/+sensitiveInviteLinkForStartupPreflight"
RAW_TDLIB_PAYLOAD_VALUE = "unit-raw-tdlib-payload-value-startup-preflight"
RAW_EXTRA = "raw-extra-startup-preflight"
RAW_TEMP_PATH = "/tmp/sensitive-startup-preflight-path"
RAW_PHONE = "+15555550123"
...
class FakeTDLibReadinessProbe:
    def __init__(
        ...
        *,
        ...
        self.request_types_sent = request_types_sent or ["getAuthorizationState"]
...
def _module():
    from scripts.ops import (
        dedicated_vps_joined_channel_collector_startup_readiness_preflight as module,
    )
...
def _runtime_env(tmp_path: Path) -> dict[str, str]:
    ...
    return {
        ...
        "JOINED_PREFLIGHT_TEMP_PATH": RAW_TEMP_PATH,
        "JOINED_PREFLIGHT_INVITE_LINK": RAW_INVITE_LINK,
    }
...
) -> tuple[dict[str, Any], FakeDatabaseConnection, FakeTDLibReadinessProbe | None]:
    module = _module()
...
def test_dry_run_reads_joined_rows_and_required_tables_without_tdlib_or_mutation(
    tmp_path: Path,
) -> None:
    report, db, probe = _run_report(tmp_path)

    assert report["contract_status"] == (
        "joined_channel_collector_startup_readiness_dry_run_ready"
    )
    ...
    assert report["live_collector_start_approved_in_this_slice"] is False
    assert db.transaction is not None
    assert db.transaction.rolled_back is True
    assert probe is None


def test_no_joined_rows_returns_blocked_no_joined_channel_rows_without_side_effects(
    tmp_path: Path,
) -> None:
    ...
    report, db, _probe = _run_report(tmp_path, db=db)
...
def test_missing_required_table_returns_blocked_required_tables_unavailable(
    tmp_path: Path,
) -> None:
    availability = {table: True for table in _module().REQUIRED_TABLES}
    availability["event_outbox"] = False
    db = FakeDatabaseConnection([_registry_row("registry-1")], table_available=availability)

    report, _db, _probe = _run_report(tmp_path, db=db)

    assert report["contract_status"] == "blocked_required_tables_unavailable"
    assert report["required_tables_available"]["event_outbox"] is False
    assert report["side_effects"]["database_mutation_performed"] is False


def test_runtime_env_unreadable_fails_closed_and_prints_no_raw_values(
    tmp_path: Path,
) -> None:
    def unreadable(_path: str | Path) -> dict[str, str]:
        raise RuntimeError(f"cannot read {FAKE_DATABASE_URL} {RAW_TEMP_PATH}")

    report, _db, _probe = _run_report(tmp_path, runtime_env_reader=unreadable)
...
def test_database_unavailable_fails_closed_and_prints_no_raw_database_url(
    tmp_path: Path,
) -> None:
...
def test_collector_import_failures_are_classified(tmp_path: Path) -> None:
    def importer(module_name: str) -> Any:
        if module_name.endswith(".runtime"):
            raise ImportError(f"broken import {FAKE_TELEGRAM_SECRET}")
        return __import__(module_name, fromlist=["*"])

    report, _db, _probe = _run_report(tmp_path, module_importer=importer)
    rendered = _module().render_json(report)

    assert report["contract_status"] == "blocked_collector_import_contract_failed"
    assert report["collector_config_import_ok"] is True
    assert report["collector_runtime_import_ok"] is False
    assert "collector_import.runtime" in report["checks_failed"]
...
def test_tdlib_readiness_probe_not_attempted_without_explicit_approval(
    ...
    report, _db, probe = _run_report(tmp_path, probe=probe, approved_tdlib=False)
...
def test_approved_tdlib_readiness_probe_reports_ready_from_fake_helper(
    ...
    report, _db, probe = _run_report(tmp_path, probe=probe, approved_tdlib=True)

    assert report["contract_status"] == (
        "joined_channel_collector_startup_readiness_tdlib_ready"
    )
...
def test_no_forbidden_pipeline_runtime_or_infra_side_effects(tmp_path: Path) -> None:
    probe = FakeTDLibReadinessProbe()

    report, _db, _probe = _run_report(tmp_path, probe=probe, approved_tdlib=True)

    side_effects = report["side_effects"]
    assert side_effects["tdlib_initialized"] is True
    ...
    assert side_effects["telegram_raw_updates_written"] is False
...
def test_help_outputs_json_cli_options_without_runtime_actions() -> None:
    ...
    assert "--joined-row-limit" in result.stdout
