...
SCRIPT = ROOT / "scripts" / "ops" / "evidence_assembler_runtime_smoke.py"
RUNBOOK = ROOT / "ops" / "pipeline" / "runbooks" / "evidence_assembler_runtime_smoke.md"
...
def test_cli_requires_confirm_write() -> None:
    module = importlib.import_module("scripts.ops.evidence_assembler_runtime_smoke")
...
def test_seed_shape_preserves_locked_colon_based_github_repo_canonical_id() -> None:
    ...
    assert seed_shape.canonical_id == "github:repo:octocat/evidence-assembler-smoke-abc123abc123"
...
def test_snapshot_updated_payload_intentionally_omits_candidate_group_id_for_fanout_path() -> None:
    module = importlib.import_module("scripts.ops.evidence_assembler_runtime_smoke")
    ...
    payload = module._build_snapshot_updated_payload(
        seed_ids=seed_ids,
        seed_shape=module._build_seed_shape("abc123abc123abc123abc123abc123ab"),
    )

    assert payload["artifact_id"] == str(seed_ids.artifact_id)
    assert payload["snapshot_id"] == str(seed_ids.snapshot_id)
    assert payload["provider"] == "github"
    assert "candidate_group_id" not in payload
...
def test_report_shape_is_stable() -> None:
    ...
    assert rendered["report_type"] == "evidence_assembler_runtime_smoke_v1"
    assert rendered["database_url_redacted"] is True
    assert rendered["redis_url_redacted"] is True
    assert rendered["queue_name"] == "q.candidate.bundle"
...
def test_runbook_documents_manual_env_and_boundaries() -> None:
    ...
    assert "q.candidate.bundle" in text
    assert "does not call external network" in text
