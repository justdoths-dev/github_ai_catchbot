...
SCRIPT = (
    ...
    / "scripts"
    / "ops"
    / "dedicated_vps_judge_openai_replay_candidate_readiness.py"
)

FAKE_DATABASE_CREDENTIAL = "unit" + "-db" + "-credential" + "-replay-candidate"
FAKE_REDIS_CREDENTIAL = "unit" + "-redis" + "-credential" + "-replay-candidate"
FAKE_DATABASE_URL = (
    "postgresql+psycopg"
    + ":"
    + "/"
...
FAKE_URL = "https" + ":" + "/" + "/" + "example.invalid" + "/private/replay-candidate"
FAKE_SOURCE_TEXT = "private source text replay candidate"
FAKE_PROMPT_CONTEXT = "private prompt context replay candidate"
...
class FakeSession:
    ...
    async def execute(self, statement: Any, params: dict[str, Any] | None = None) -> FakeResult:
        ...
        if normalized == _normalize(module.SELECT_ANALYSIS_REQUESTED_EVENTS_FOR_BUNDLE_QUERY):
            rows = [
                row
                for row in self.analysis_rows
                if str(row["payload_json"].get("bundle_id")) == str(params["bundle_id"])
            ]
            return FakeResult(rows=rows[: int(params["limit"])])
        ...
        if normalized == _normalize(module.SELECT_HISTORICAL_TERMINAL_JUDGE_RUNS_FOR_BUNDLE_QUERY):
            suffix = module.REPLAY_PROMPT_SUFFIX
            rows = [
                row
                for row in self.judge_runs.values()
                if str(row["bundle_id"]) == str(params["bundle_id"])
                and row["status"] in {"succeeded", "failed_terminal"}
                and not str(row["prompt_version"]).endswith(suffix)
            ]
            return FakeResult(rows=rows[: int(params["limit"])])
...
def _module():
    return importlib.import_module(
        "scripts.ops.dedicated_vps_judge_openai_replay_candidate_readiness"
    )
...
def _approvals(**overrides: bool) -> Any:
    values = {"db_write": False, "redis_publish": False, "replay_candidate": False}
    values.update(overrides)
    return _module().ReplayCandidateApprovals(**values)


def _all_approvals() -> Any:
    return _approvals(db_write=True, redis_publish=True, replay_candidate=True)
    ...
    *,
    ...
    return {
        ...
        "prompt_cache_key": (
            ...
            + prompt_version
            + ":judge_output_v1:verdict_policy_v1"
        ),
        "status": status,
    }
...
def _session_with_analysis_candidate(
    ...
    **kwargs: Any,
    ...
    if judge_runs is None:
        historical = _judge_run_row(bundle, status="succeeded")
        judge_runs = {str(historical["judge_run_id"]): historical}
    return FakeSession(
        analysis_rows=rows,
        bundle_rows=[bundle],
...
def test_default_mode_is_read_only_and_performs_no_db_write_redis_mutation_or_openai() -> None:
    ...
    assert result.report["replay_candidate_found_bucket"] == "one"
...
def test_missing_approval_pair_blocks_before_connections() -> None:
    ...
    assert "approval.replay_candidate" in result.report["checks_failed"]
...
def test_zero_eligible_candidate_blocks() -> None:
    ...
    assert result.report["replay_candidate_found_bucket"] == "zero"
    assert redis.xadd_calls == []
    assert session.inserted_judge_runs == []
...
def test_multiple_eligible_candidates_blocks_as_ambiguous() -> None:
    ...
    session = FakeSession(
        bundle_rows=[first_bundle, second_bundle],
        bundles={
        ...
        judge_runs={
            str(uuid4()): _judge_run_row(first_bundle, status="succeeded"),
            str(uuid4()): _judge_run_row(second_bundle, status="succeeded"),
        },
    )
    ...
    assert result.report["current_ready_bundle_found_bucket"] == "multiple"
    assert redis.xadd_calls == []
    assert session.inserted_judge_runs == []
...
def test_existing_active_q_analysis_judge_candidate_blocks_duplicate_target() -> None:
    bundle = _bundle_row()
    judge_run = _judge_run_row(bundle, status="pending", replay=True)
...
def test_no_historical_terminal_judge_run_blocks() -> None:
    bundle = _bundle_row()
    result, session, redis = _run_report(
        session=_session_with_analysis_candidate(bundle=bundle, judge_runs={})
    )

    assert result.exit_code == 1
    assert result.report["contract_status"] == _module().STATUS_NO_CANDIDATE
    assert result.report["historical_terminal_judge_run_found_bucket"] == "zero"
    assert redis.xadd_calls == []
    assert session.inserted_judge_runs == []


def test_running_only_historical_run_does_not_make_replay_candidate() -> None:
    bundle = _bundle_row()
...
def test_failed_retryable_only_historical_run_does_not_make_replay_candidate() -> None:
    bundle = _bundle_row()
    failed_retryable = _judge_run_row(bundle, status="failed_retryable")

    result, session, redis = _run_report(
        session=_session_with_analysis_candidate(
            bundle=bundle,
            judge_runs={str(failed_retryable["judge_run_id"]): failed_retryable},
        )
    )
...
def test_succeeded_historical_run_remains_replay_eligible() -> None:
    bundle = _bundle_row()
    succeeded = _judge_run_row(bundle, status="succeeded")

    result, session, redis = _run_report(
        session=_session_with_analysis_candidate(
            bundle=bundle,
            judge_runs={str(succeeded["judge_run_id"]): succeeded},
        )
    )

    assert result.exit_code == 0
    assert result.report["contract_status"] == _module().STATUS_PREFLIGHT_PASSED
    assert result.report["historical_terminal_judge_run_found_bucket"] == "one"
    assert redis.xadd_calls == []
    assert session.inserted_judge_runs == []


def test_failed_terminal_historical_run_remains_replay_eligible() -> None:
    bundle = _bundle_row()
    failed_terminal = _judge_run_row(bundle, status="failed_terminal")

    result, session, redis = _run_report(
        session=_session_with_analysis_candidate(
            bundle=bundle,
            judge_runs={str(failed_terminal["judge_run_id"]): failed_terminal},
        )
    )
...
def test_existing_replay_pending_running_or_succeeded_judge_run_blocks_without_reuse() -> None:
    for status in ("pending", "running", "succeeded"):
        bundle = _bundle_row()
        historical = _judge_run_row(bundle, status="succeeded")
        replay = _judge_run_row(bundle, status=status, replay=True)
        result, session, redis = _run_report(
            session=_session_with_analysis_candidate(
                bundle=bundle,
                judge_runs={
                    str(historical["judge_run_id"]): historical,
                    str(replay["judge_run_id"]): replay,
                },
            ),
        ...
        assert result.exit_code == 1
        assert result.report["contract_status"] == _module().STATUS_EXISTING_REPLAY_RUN
        assert result.report["existing_replay_judge_run_bucket"] == "one"
        assert session.inserted_judge_runs == []
        assert session.inserted_outbox == []
        assert redis.xadd_calls == []


def test_existing_replay_judge_output_blocks() -> None:
    bundle = _bundle_row()
    historical = _judge_run_row(bundle, status="succeeded")
    replay = _judge_run_row(bundle, status="failed_terminal", replay=True)
    result, session, redis = _run_report(
        session=_session_with_analysis_candidate(
            bundle=bundle,
            judge_runs={
            ...
            output_counts={str(replay["judge_run_id"]): 1},
        )
    )

    assert result.exit_code == 1
    assert result.report["contract_status"] == _module().STATUS_EXISTING_REPLAY_OUTPUT
    assert result.report["existing_replay_output_bucket"] == "one"
    assert redis.xadd_calls == []
    assert session.inserted_outbox == []


def test_existing_replay_judge_output_ready_outbox_blocks() -> None:
    bundle = _bundle_row()
    historical = _judge_run_row(bundle, status="succeeded")
    replay = _judge_run_row(bundle, status="failed_terminal", replay=True)
    result, session, redis = _run_report(
        session=_session_with_analysis_candidate(
            ...
            ready_outbox_counts={str(replay["judge_run_id"]): 1},
        )
    )

    assert result.exit_code == 1
    assert result.report["contract_status"] == _module().STATUS_EXISTING_REPLAY_READY_OUTBOX
    assert result.report["existing_replay_ready_outbox_bucket"] == "one"
    assert redis.xadd_calls == []
    assert session.inserted_outbox == []
...
def test_approved_mode_creates_one_pending_judge_run_and_one_judge_call_outbox() -> None:
    ...
    assert result.report["replay_judge_run_created_bucket"] == "one"
    assert result.report["replay_judge_call_requested_outbox_created_bucket"] == "one"
    assert len(session.inserted_judge_runs) == 1
    assert session.inserted_judge_runs[0]["status"] == "pending"
    assert session.inserted_judge_runs[0]["prompt_version"].endswith(
        "__replay_live_smoke_v1"
    )
    assert session.inserted_judge_runs[0]["prompt_cache_key"].endswith(
        "judge_output_v1:verdict_policy_v1"
    )
    assert len(session.inserted_outbox) == 1
    assert session.inserted_outbox[0]["event_type"] == "judge.call.requested.v1"
    assert session.inserted_outbox[0]["payload_json"]["replay_reason_code"] == (
        "manual_live_smoke_replay"
    )
    assert redis.xadd_calls


def test_replay_prompt_version_preserves_historical_base_and_appends_suffix() -> None:
    module = _module()
    bundle = _bundle_row()
    historical = _judge_run_row(bundle, status="succeeded")
    historical["prompt_version"] = "custom_github_prompt_v7"
    historical["prompt_cache_key"] = (
        "judge:github_primary:custom_github_prompt_v7:judge_output_v1:"
        "verdict_policy_v1"
    )
    session = _session_with_analysis_candidate(
        bundle=bundle,
    ...
    def runtime_env(_path: str | Path) -> dict[str, str]:
        values = _runtime_env(_path)
        values["JUDGE_PROMPT_VERSION_GITHUB"] = "ignored_runtime_prompt"
        return values

    result = module.generate_report(
        runtime_env_path=FAKE_RUNTIME_PATH,
        approvals=_all_approvals(),
        runtime_env_reader=runtime_env,
        database_session_factory=lambda _url: session,
    ...
    assert result.exit_code == 0
    assert session.inserted_judge_runs[0]["prompt_version"] == (
        "custom_github_prompt_v7__replay_live_smoke_v1"
    )
    assert session.inserted_judge_runs[0]["prompt_cache_key"] == (
        "judge:github_primary:custom_github_prompt_v7__replay_live_smoke_v1:"
        "judge_output_v1:verdict_policy_v1"
    )


def test_analysis_requested_profile_is_used_when_historical_profile_is_not_allowed() -> None:
    bundle = _bundle_row(artifact_type="github_repo")
    event = _analysis_event(bundle)
    event["payload_json"]["judge_profile"] = "x_primary"
    historical = _judge_run_row(bundle, status="succeeded")
    historical["judge_profile"] = "legacy_profile"
    session = _session_with_analysis_candidate(
        bundle=bundle,
        analysis_rows=[event],
        judge_runs={str(historical["judge_run_id"]): historical},
    ...
    result, session, _redis = _run_report(session=session, approvals=_all_approvals())

    assert result.exit_code == 0
    assert session.inserted_judge_runs[0]["judge_profile"] == "x_primary"
    assert session.inserted_judge_runs[0]["prompt_version"] == (
        "judge_x_primary_v1__replay_live_smoke_v1"
    )
