...
SCRIPT = (
    ...
    / "scripts"
    / "ops"
    / "dedicated_vps_maintenance_delivery_result_dry_run_noop_smoke.py"
)

FAKE_DATABASE_CREDENTIAL = "unit" + "-db" + "-credential" + "-maintenance-noop"
FAKE_DATABASE_URL = (
    "postgresql+psycopg"
    + ":"
    + "/"
...
FAKE_RUNTIME_PATH = "/etc/github-ai-catchbot/private-maintenance-noop-runtime.env"
FAKE_BOT_TOKEN = "private" + "-telegram" + "-bot" + "-maintenance-noop"
FAKE_OPENAI_KEY = "direct" + "-openai" + "-secret" + "-maintenance-noop"
FAKE_REDIS_URL = "redis" + ":/" + "/private-maintenance-noop"
FAKE_URL = "https" + ":" + "/" + "/" + "example.invalid/private/maintenance-noop"
FAKE_SOURCE_TEXT = "private source text maintenance noop must not leak"
FAKE_MESSAGE_BODY = "private rendered telegram body maintenance noop must not leak"
...
class FakeSession:
    def __init__(
        ...
        *,
        ...
        delivery_reason: str | None = "dry_run_skip_transport",
        existing_maintenance_marker_count: int = 0,
        ...
        self.bundle_fingerprint = "candidate-bundle-fingerprint-private-maintenance"
        self.include_target = include_target
        self.target_count = target_count
        self.delivery_status = delivery_status
        self.delivery_reason = delivery_reason
        self.existing_maintenance_marker_count = existing_maintenance_marker_count
    ...
    async def execute(self, statement: Any, params: dict[str, Any] | None = None) -> FakeResult:
        ...
        if normalized == _normalize(module.SELECT_DRY_RUN_DELIVERY_RESULT_TARGETS_QUERY):
        ...
        if normalized == _normalize(module.COUNT_POLICY_STATE_TRANSITIONS_FOR_TARGET_QUERY):
            return FakeResult(scalar=1 + len(self.policy_transition_mutations))
        ...
        if normalized == _normalize(module.COUNT_CANDIDATE_ROWS_FOR_TARGET_QUERY):
            return FakeResult(scalar=1)
        if normalized == _normalize(module.COUNT_MAINTENANCE_JOB_ATTEMPTS_FOR_TARGET_QUERY):
            return FakeResult(scalar=self.existing_maintenance_marker_count + len(self.job_attempts))
...
def _module():
    return importlib.import_module(
        "scripts.ops.dedicated_vps_maintenance_delivery_result_dry_run_noop_smoke"
    )
...
def test_db_read_preflight_finds_existing_dry_run_result_and_writes_nothing() -> None:
    ...
    assert report["target_delivery_status_bucket"] == "suppressed"
    assert report["target_delivery_reason_bucket"] == "dry_run_skip_transport"
    ...
    assert report["existing_maintenance_job_attempt_for_target_bucket"] == "zero"
    assert report["existing_retry_intent_outbox_for_target_bucket"] == "zero"
    assert report["existing_dead_letter_for_target_bucket"] == "zero"
    assert report["existing_replay_request_for_target_bucket"] == "zero"
    assert report["maintenance_classification_bucket"] == "logical_noop_success"
...
def test_db_read_preflight_rejects_missing_target() -> None:
    result, session = _run_report(session=FakeSession(include_target=False))
...
def test_db_read_preflight_rejects_multiple_targets() -> None:
    result, session = _run_report(session=FakeSession(target_count=2))
...
def test_db_read_preflight_rejects_wrong_status_or_reason() -> None:
    result, session = _run_report(
        session=FakeSession(delivery_status="failed_retryable", delivery_reason="telegram_retryable_5xx")
    )
    ...
    assert result.report["target_delivery_reason_bucket"] == "telegram_retryable_5xx"
    assert result.report["maintenance_classification_bucket"] == "out_of_scope"
    assert result.report["database_write_attempted"] is False
    _assert_no_writes(session)


def test_db_read_preflight_rejects_existing_maintenance_marker() -> None:
    result, session = _run_report(session=FakeSession(existing_maintenance_marker_count=1))

    assert result.exit_code == 1
    assert result.report["contract_status"] == _module().STATUS_EXISTING_MARKER
    assert result.report["checks_failed"] == ["maintenance_job_attempt.existing"]
    assert result.report["existing_maintenance_job_attempt_for_target_bucket"] == "one"
    _assert_no_writes(session)


def test_db_read_preflight_rejects_existing_retry_dead_letter_or_replay_effect() -> None:
    result, session = _run_report(session=FakeSession(existing_retry_intent_count=1))
...
def test_db_write_fake_success_writes_one_maintenance_marker_only() -> None:
    ...
    assert attempt["root_object_type"] == "notification_plan"
    assert str(attempt["root_object_id"]) == str(session.notification_plan_id)
    assert attempt["attempt_status"] == "succeeded"
    assert attempt["error_code"] == "delivery_result_suppressed_dry_run_noop"
