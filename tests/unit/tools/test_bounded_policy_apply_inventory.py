...
from tools import bounded_policy_apply_inventory as runner


ROOT = Path(__file__).resolve().parents[3]
TOOL_PATH = ROOT / "tools/bounded_policy_apply_inventory.py"
...
def test_main_with_no_flags_returns_required_fail_closed_json_and_empty_stderr(capsys) -> None:
    ...
    assert exit_code == 1
    assert captured.err == ""
    assert parsed["schema_version"] == "bounded_policy_apply_inventory_v1"
    assert parsed["runner_name"] == "bounded_policy_apply_inventory"
    assert parsed["mode"] == "policy_apply_db_inventory_read_only"
...
def test_invalid_limits_and_prefer_verdict_return_json_without_runtime_config() -> None:
    cases = (
        (["--operator-approved", "--db-limit", "0"], "invalid_db_limit"),
        (["--operator-approved", "--db-limit", "501"], "invalid_db_limit"),
        (["--operator-approved", "--redis-scan-limit", "0"], "invalid_redis_scan_limit"),
        (["--operator-approved", "--redis-scan-limit", "501"], "invalid_redis_scan_limit"),
        (["--operator-approved", "--max-results", "0"], "invalid_max_results"),
        (["--operator-approved", "--max-results", "51"], "invalid_max_results"),
