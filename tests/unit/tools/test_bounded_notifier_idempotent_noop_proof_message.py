...
from tools import bounded_notifier_idempotent_noop_proof_message as runner
from tests.unit.services.notifier_telegram.test_idempotent_noop_proof_message import (
    FakeRuntime,
    FakeRuntimeBuilder,
    _base_config,
    _intent,
...
TOOL_PATH = ROOT / "tools/bounded_notifier_idempotent_noop_proof_message.py"
...
def test_main_with_no_flags_returns_required_fail_closed_json_and_empty_stderr(capsys) -> None:
    ...
    assert exit_code == 1
    assert captured.err == ""
    assert parsed["schema_version"] == "bounded_notifier_idempotent_noop_proof_message_v1"
    assert parsed["runner_name"] == "bounded_notifier_idempotent_noop_proof_message"
...
def test_preview_cli_delegates_once_and_omits_raw_ids_or_secrets(capsys) -> None:
    ...
    assert parsed["proof_publish_safe"] is True
    assert parsed["proof_message_published"] is False
...
def test_publish_cli_delegates_one_xadd_and_returns_only_message_id_suffix(capsys) -> None:
    intent = _intent()
    runtime = FakeRuntime(intents=[intent])
    argv = _preview_argv(intent) + [
        "--mode",
        "publish",
        "--allow-redis-publish",
        "--allow-proof-message-publish",
    ]
    del argv[argv.index("preview") - 1 : argv.index("preview") + 1]
    ...
    assert exit_code == 0
    assert parsed["status"] == "pass"
    assert parsed["proof_message_published"] is True
    assert parsed["proof_message_id_suffix"] == "011048-0"
    assert len(runtime.publish_calls) == 1
    assert "1718000011048-0" not in output


def test_unsupported_live_ack_consume_or_raw_authority_flags_return_sanitized_json(capsys) -> None:
    for flag in (
        "--allow-telegram",
        "--allow-send",
        "--allow-redis-ack",
...
def test_tool_source_imports_no_db_redis_external_clients_or_forbidden_flags() -> None:
