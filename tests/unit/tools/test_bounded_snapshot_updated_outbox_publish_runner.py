...
from src.services.outbox_relay import bounded_snapshot_updated_outbox_publish_runner
...
from tools import bounded_snapshot_updated_outbox_publish_runner as runner


ROOT = Path(__file__).resolve().parents[3]
TOOL_PATH = ROOT / "tools/bounded_snapshot_updated_outbox_publish_runner.py"
REDIS_LOCATOR = "redis_locator_omitted_sentinel"
DB_LOCATOR = "db_locator_omitted_sentinel"
RAW_DEDUPE_KEY = "artifact:snapshot:cli-sentinel-dedupe-key"
RAW_FINAL_URL = "https://example.invalid/cli-private-final-url"
RAW_ARTICLE_TEXT = "sentinel cli article text"
RAW_TITLE = "sentinel cli title"
RAW_DESCRIPTION = "sentinel cli description"
RAW_CONTENT_ANCHOR = "sentinel-cli-content-anchor"
REDIS_MESSAGE_ID = "secret-cli-snapshot-updated-redis-message-id"
...
def _payload(artifact_id) -> dict[str, object]:
    return {
...
def test_runner_uses_source_level_module_and_config_types() -> None:
    assert runner.BoundedSnapshotUpdatedOutboxPublishConfig is (
        bounded_snapshot_updated_outbox_publish_runner.BoundedSnapshotUpdatedOutboxPublishConfig
    )
    assert runner.BoundedSnapshotUpdatedPublishRuntimeConfig is (
        bounded_snapshot_updated_outbox_publish_runner.BoundedSnapshotUpdatedPublishRuntimeConfig
    )
...
def test_main_with_no_flags_returns_required_fail_closed_json_and_empty_stderr(capsys) -> None:
    ...
    assert exit_code == 1
    assert captured.err == ""
    assert parsed["schema_version"] == "bounded_snapshot_updated_outbox_publish_v1"
    assert parsed["runner_name"] == "bounded_snapshot_updated_outbox_publish_runner"
    assert parsed["mode"] == "snapshot_updated_outbox_one_shot_publish"
    assert parsed["gates"] == {
    ...
    assert parsed["side_effects"]["router_called"] is False
...
def test_invalid_uuid_or_suffix_returns_sanitized_json_without_runtime_config(capsys) -> None:
    ...
    invalid_artifact_id_exit = runner.main(["--operator-approved", "--artifact-id", "not-a-uuid"])
    invalid_artifact_id = json.loads(capsys.readouterr().out)
    ...
    assert invalid_artifact_id_exit == 1
    assert invalid_artifact_id["error_code"] == "invalid_artifact_id"
...
def test_valid_cli_fake_run_prints_json_only_and_delegates_to_source(capsys) -> None:
    ...
    assert parsed["selected_payload_provider"] == "web"
    assert parsed["selected_payload_provider_route"] == "web"
...
def test_run_with_explicit_args_uses_artifact_selector() -> None:
    row = _row()
    repository = FakeRepository(row)
    publisher = FakePublisher()
    ...
    assert result.report["selector_type"] == "artifact_id"
    assert result.report["target_artifact_id_suffix"] == str(row.aggregate_id)[-8:]
    assert repository.fetch_calls[0]["artifact_id"] == row.aggregate_id
