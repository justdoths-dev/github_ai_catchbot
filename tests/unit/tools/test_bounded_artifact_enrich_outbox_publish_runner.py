...
from src.services.outbox_relay import bounded_artifact_enrich_outbox_publish_runner
...
from tools import bounded_artifact_enrich_outbox_publish_runner as runner


ROOT = Path(__file__).resolve().parents[3]
TOOL_PATH = ROOT / "tools/bounded_artifact_enrich_outbox_publish_runner.py"
REDIS_LOCATOR = "redis_locator_omitted_sentinel"
DB_LOCATOR = "db_locator_omitted_sentinel"
RAW_DEDUPE_KEY = "artifact:enrich:cli-sentinel-dedupe-key"
RAW_URL = "https://example.invalid/cli-sentinel-url"
RAW_ARTICLE_TEXT = "sentinel cli article text"
REDIS_MESSAGE_ID = "secret-cli-artifact-enrich-redis-message-id"
...
def test_runner_uses_source_level_module_and_config_types() -> None:
    assert runner.BoundedArtifactEnrichOutboxPublishConfig is (
        bounded_artifact_enrich_outbox_publish_runner.BoundedArtifactEnrichOutboxPublishConfig
    )
    assert runner.BoundedArtifactEnrichPublishRuntimeConfig is (
        bounded_artifact_enrich_outbox_publish_runner.BoundedArtifactEnrichPublishRuntimeConfig
    )
...
def test_main_with_no_flags_returns_required_fail_closed_json_and_empty_stderr(capsys) -> None:
    ...
    assert exit_code == 1
    assert captured.err == ""
    assert parsed["schema_version"] == "bounded_artifact_enrich_outbox_publish_v1"
    assert parsed["runner_name"] == "bounded_artifact_enrich_outbox_publish_runner"
    assert parsed["mode"] == "artifact_enrich_outbox_one_shot_publish"
    assert parsed["gates"] == {
    ...
    assert parsed["side_effects"]["normalizer_called"] is False
    assert parsed["side_effects"]["enricher_called"] is False
...
def test_valid_cli_fake_run_prints_json_only_and_delegates_to_source(capsys) -> None:
    ...
    assert parsed["selected_provider_route"] == "web"
...
def test_tool_ast_guard_has_no_forbidden_process_or_network_calls() -> None:
    source = TOOL_PATH.read_text(encoding="utf-8")
    tree = ast.parse(source)
