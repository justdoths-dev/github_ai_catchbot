...
def test_smoke_module_imports_without_db_redis_openai_or_network_connection(monkeypatch) -> None:
    calls: list[str] = []
    module_name = "scripts.ops.delivery_gate_full_runtime_smoke"
...
def test_help_works_without_live_db_redis_openai_or_network() -> None:
    ...
    assert "GITHUB_AI_CATCHBOT_DB_SMOKE_DATABASE_URL" in normalized_output
    assert "Redis is not required" in normalized_output
