...
def test_smoke_module_imports_without_db_redis_or_network_connection(monkeypatch) -> None:
    calls: list[str] = []
    module_name = "scripts.ops.evidence_assembler_runtime_smoke"
    ...
    try:
        ...
        assert module.REPORT_TYPE == "evidence_assembler_runtime_smoke_v1"
        assert calls == []
    finally:
        sys.modules.pop(module_name, None)


def test_help_works_without_live_db_redis_or_network() -> None:
