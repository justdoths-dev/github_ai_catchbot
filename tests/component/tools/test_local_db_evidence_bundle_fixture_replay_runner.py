...
pytestmark = pytest.mark.skipif(
    ...
    reason="LOCAL_TEST_DATABASE_URL is required for the DB-backed EvidenceBundle replay component test",
)


def test_cli_writes_fixture_backed_evidence_bundle_after_github_snapshot_replay() -> None:
    database_url = os.environ["LOCAL_TEST_DATABASE_URL"]
    namespace = f"component-local-db-evidence-bundle-{uuid4().hex}"
    ...
    assert after["candidate_evidence_bundles"] - before["candidate_evidence_bundles"] in {0, 1}
    assert after["candidate_evidence_members"] - before["candidate_evidence_members"] in {0, 1}
    assert after["analysis_requested_events"] - before["analysis_requested_events"] in {0, 1}
    for table in (
        "judge_runs",
        "judge_outputs",
        "analyses",
...
def _run_cli(*, database_url: str, namespace: str) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(
        [
            sys.executable,
            "-m",
            "tools.local_db_evidence_bundle_fixture_replay_runner",
            "--database-url",
            database_url,
            "--source-fixture",
...
def _fetch_durable_rows(*, database_url: str, namespace: str) -> dict[str, int]:
    ...
    try:
        with engine.connect() as connection:
            ...
            analysis_dedupe = (
                runner.build_analysis_requested_dedupe_key(
                    replay_namespace=namespace,
                    candidate_group_id=candidate_group_id,
                    bundle_id=bundle_id,
                )
                if candidate_group_id and bundle_id
                else ""
            )
            return {
                ...
                "snapshot_updated_outbox_events": _count(
                    ...
                    """,
                    {
                        ...
                        "artifact_id": str(artifact_id),
                        "dedupe_key": github_runner.build_snapshot_updated_dedupe_key(
                            replay_namespace=namespace,
                ...
                "analysis_requested_events": _count(
                    ...
                    WHERE event_type = :event_type
                      AND dedupe_key = :dedupe_key
                    """,
                    {"event_type": runner.ANALYSIS_REQUESTED_EVENT_TYPE, "dedupe_key": analysis_dedupe},
                ),
                "judge_runs": _count(
                    connection,
                    """
