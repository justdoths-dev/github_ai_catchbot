...
pytestmark = pytest.mark.skipif(
    ...
    reason="LOCAL_TEST_DATABASE_URL is required for the DB-backed analysis-validator replay component test",
)


def test_cli_writes_fixture_backed_policy_apply_after_fake_judge_output_replay() -> None:
    database_url = os.environ["LOCAL_TEST_DATABASE_URL"]
    namespace = f"component-local-db-analysis-validator-{uuid4().hex}"
    ...
    first_transition = _fetch_validation_transition(database_url=database_url, namespace=namespace)
    first_policy_payload = _fetch_policy_apply_payload(database_url=database_url, namespace=namespace)
    ...
    second_transition = _fetch_validation_transition(database_url=database_url, namespace=namespace)
    second_policy_payload = _fetch_policy_apply_payload(database_url=database_url, namespace=namespace)
    ...
    assert second_policy_payload == first_policy_payload
    ...
    assert first_transition == {
        ...
        "reason_code": "judge_output_validated",
    }
    assert set(first_policy_payload) == {"judge_run_id", "judge_output_id", "candidate_group_id", "bundle_id"}
    ids = _fetch_ids(database_url=database_url, namespace=namespace)
    assert first_policy_payload == {
    ...
    assert after_global["state_transitions"] - before_global["state_transitions"] in {0, 1}
    assert after_global["namespace_analysis_policy_apply_events"] == 1
    assert before_global["namespace_analysis_policy_apply_events"] == 0
...
def _run_cli(*, database_url: str, namespace: str) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(
        [
            sys.executable,
            "-m",
            "tools.local_db_analysis_validator_fixture_replay_runner",
            "--database-url",
            database_url,
            "--source-fixture",
...
def _fetch_durable_rows(*, database_url: str, namespace: str) -> dict[str, int]:
    ...
    policy_dedupe = runner.build_analysis_policy_apply_dedupe_key(
        replay_namespace=namespace,
        judge_run_id=ids["judge_run_id"],
        judge_output_id=ids["judge_output_id"],
    )
        ...
        with engine.connect() as connection:
            return {
                ...
                "analysis_policy_apply_events": _count(
                    ...
                    """,
                    {
                        "event_type": runner.ANALYSIS_POLICY_APPLY_EVENT_TYPE,
                        "judge_run_id": ids["judge_run_id"],
...
def _fetch_validation_transition(*, database_url: str, namespace: str) -> dict[str, str | None]:
    ids = _fetch_ids(database_url=database_url, namespace=namespace)
    engine = sa.create_engine(database_url, future=True)
    try:
        with engine.connect() as connection:
...
def _fetch_policy_apply_payload(*, database_url: str, namespace: str) -> dict[str, str]:
    ids = _fetch_ids(database_url=database_url, namespace=namespace)
    dedupe_key = runner.build_analysis_policy_apply_dedupe_key(
        replay_namespace=namespace,
        judge_run_id=ids["judge_run_id"],
        judge_output_id=ids["judge_output_id"],
    )
...
def _fetch_global_downstream_counts(database_url: str, *, namespace: str) -> dict[str, int]:
    ...
    try:
        with engine.connect() as connection:
            return {
                "state_transitions": _count(connection, "SELECT count(*) FROM state_transitions", {}),
                "namespace_analysis_policy_apply_events": _count(
                    connection,
                    """
                    SELECT count(*)
                    FROM event_outbox
                    ...
                    """,
                    {"dedupe_prefix": f"local-db-analysis-validator:{namespace}:analysis.policy.apply:%"},
                ),
