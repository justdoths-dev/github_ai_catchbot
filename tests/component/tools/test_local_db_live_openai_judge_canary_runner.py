...
FAKE_API_KEY = "component_fake_openai_key"
EVENT_NAME_FILES = (
...
class FakeResponsesClient:
    ...
    def create(self, **request):
        ...
        bundle = runner.restricted_runner.BundleContext(
        ...
        payload = runner.restricted_runner.build_fake_judge_output_payload(bundle)
        return {
            "status": "completed",
...
class FakeLiveClientFactory:
    ...
    def __call__(self, *, api_key: str) -> FakeLiveOpenAIClient:
        self.api_keys.append(api_key)
        client = FakeLiveOpenAIClient()
        self.clients.append(client)
        return client


def test_local_db_live_openai_judge_canary_uses_fake_live_client_and_is_idempotent() -> None:
    ...
    namespace = f"component-local-db-live-openai-judge-{uuid4().hex}"
    factory = FakeLiveClientFactory()
    before_global = _fetch_global_downstream_counts(database_url)

    first = _run_canary(database_url=database_url, namespace=namespace, factory=factory)
    first_ids = _fetch_ids(database_url=database_url, namespace=namespace)
    first_judge = _fetch_judge_run_fields(database_url=database_url, judge_run_id=first_ids["judge_run_id"])
    first_output = _fetch_judge_output_fields(database_url=database_url, judge_output_id=first_ids["judge_output_id"])
    first_ready_payload = _fetch_ready_event_payload(database_url=database_url, namespace=namespace, ids=first_ids)
    first_scoped_counts = _fetch_scoped_counts(database_url=database_url, namespace=namespace, ids=first_ids)

    second = _run_canary(database_url=database_url, namespace=namespace, factory=factory)
    second_ids = _fetch_ids(database_url=database_url, namespace=namespace)
    second_judge = _fetch_judge_run_fields(database_url=database_url, judge_run_id=second_ids["judge_run_id"])
    second_output = _fetch_judge_output_fields(database_url=database_url, judge_output_id=second_ids["judge_output_id"])
    second_scoped_counts = _fetch_scoped_counts(database_url=database_url, namespace=namespace, ids=second_ids)
    after_global = _fetch_global_downstream_counts(database_url)

    assert first.exit_code == 0
    assert second.exit_code == 0
    assert first.report == _expected_first_pass_report()
    assert second.report["status"] == "pass"
    assert second.report["existing_judge_output_reused"] is True
    assert second.report["openai_client_created"] is False
    assert second.report["live_openai_called"] is False
    assert second.report["checks_failed"] == []
    assert factory.api_keys == [FAKE_API_KEY]
    assert len(factory.clients) == 1
    assert len(factory.clients[0].responses.calls) == 1

    request = factory.clients[0].responses.calls[0]
    user_context = json.loads(request["input"][1]["content"][0]["text"])
    ...
    assert request["max_output_tokens"] == runner.DEFAULT_MAX_OUTPUT_TOKENS
    assert request["text"]["format"]["type"] == "json_schema"
    assert request["text"]["format"]["name"] == "judge_output_v1"
    assert request["text"]["format"]["strict"] is True
    assert set(user_context) == set(runner.restricted_runner.REQUEST_CONTEXT_KEYS)
    request_text = json.dumps(request)
    assert "source_messages" not in request_text
    assert "artifact_snapshots" not in request_text
    assert "web_search" not in request_text
    assert "file_search" not in request_text
    assert database_url not in request_text
    assert FAKE_API_KEY not in request_text
    ...
    assert first_ready_payload == {
        "judge_run_id": first_ids["judge_run_id"],
        "judge_output_id": first_ids["judge_output_id"],
        "finish_reason": "stop",
        "refusal_detected": False,
    }
    assert first_scoped_counts["judge_outputs"] == 1
    assert first_scoped_counts["judge_output_ready_events"] == 1
    assert second_scoped_counts == first_scoped_counts
    ...
    for key in (
        ...
        assert after_global[key] == before_global[key]
    assert first.report["telegram_called"] is False
    assert first.report["live_github_called"] is False
    assert first.report["workers_started"] is False
    assert first.report["redis_mutation"] is False
    assert first.report["production_db_write"] is False
    assert first.report["alembic_or_ddl_ran"] is False
    assert first.report["analysis_created"] is False
    assert first.report["notification_created"] is False
    _assert_no_event_name_files()


def _run_canary(*, database_url: str, namespace: str, factory: FakeLiveClientFactory) -> runner.RunnerResult:
    ...
    return runner.run(
        args,
        env={"APP_ENV": "test", "OPENAI_API_KEY": FAKE_API_KEY},
...
def _fetch_ids(*, database_url: str, namespace: str) -> dict[str, str]:
    ...
    try:
        with engine.connect() as connection:
            ...
            ready_payload_json = json.loads(ready_payload) if isinstance(ready_payload, str) else dict(ready_payload or {})
...
def _fetch_judge_run_fields(*, database_url: str, judge_run_id: str) -> dict[str, object]:
    engine = sa.create_engine(database_url, future=True)
    try:
        with engine.connect() as connection:
...
def _fetch_judge_output_fields(*, database_url: str, judge_output_id: str) -> dict[str, object]:
    engine = sa.create_engine(database_url, future=True)
    try:
        with engine.connect() as connection:
            row = connection.execute(
...
def _fetch_ready_event_payload(*, database_url: str, namespace: str, ids: dict[str, str]) -> dict[str, object]:
    dedupe_key = runner.restricted_runner.build_judge_output_ready_dedupe_key(
...
def _fetch_scoped_counts(*, database_url: str, namespace: str, ids: dict[str, str]) -> dict[str, int]:
    ready_dedupe = runner.restricted_runner.build_judge_output_ready_dedupe_key(
        replay_namespace=namespace,
        judge_run_id=UUID(ids["judge_run_id"]),
        judge_output_id=UUID(ids["judge_output_id"]),
    )
        ...
        with engine.connect() as connection:
            return {
                "analysis_requested_events": _count(
                    connection,
                    "SELECT count(*) FROM event_outbox WHERE event_type = 'analysis.requested.v1' AND dedupe_key = :dedupe_key",
                    {"dedupe_key": analysis_dedupe},
                ),
                "judge_call_requested_events": _count(
                    connection,
                    "SELECT count(*) FROM event_outbox WHERE event_type = 'judge.call.requested.v1' AND dedupe_key = :dedupe_key",
                    {"dedupe_key": judge_dedupe},
                ),
                "judge_outputs": _count(
                    connection,
                    "SELECT count(*) FROM judge_outputs WHERE judge_output_id = CAST(:judge_output_id AS uuid)",
                    {"judge_output_id": ids["judge_output_id"]},
                ),
                "judge_output_ready_events": _count(
                    connection,
                    "SELECT count(*) FROM event_outbox WHERE event_type = 'judge.output.ready.v1' AND dedupe_key = :dedupe_key",
                    {"dedupe_key": ready_dedupe},
                ),
                "analyses": _count(
                    connection,
                    "SELECT count(*) FROM analyses WHERE candidate_group_id = CAST(:candidate_group_id AS uuid)",
                    {"candidate_group_id": ids["candidate_group_id"]},
                ),
                "notification_plans": _count(
                    connection,
                    "SELECT count(*) FROM notification_plans WHERE candidate_group_id = CAST(:candidate_group_id AS uuid)",
                    {"candidate_group_id": ids["candidate_group_id"]},
                ),
...
def _fetch_global_downstream_counts(database_url: str) -> dict[str, int]:
    engine = sa.create_engine(database_url, future=True)
    try:
        with engine.connect() as connection:
            return {
...
def _expected_first_pass_report() -> dict[str, object]:
    return {
