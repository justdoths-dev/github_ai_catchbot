...
pytestmark = pytest.mark.skipif(
    ...
    reason="LOCAL_TEST_DATABASE_URL is required for the DB-backed fake judge output replay component test",
)


def test_cli_writes_fixture_backed_fake_judge_output_after_analysis_router_replay() -> None:
    database_url = os.environ["LOCAL_TEST_DATABASE_URL"]
    namespace = f"component-local-db-fake-judge-output-{uuid4().hex}"
    ...
    assert first_judge["model"] == "gpt-5.4-mini"
    assert first_judge["reasoning_effort"] == "low"
    assert first_judge["prompt_version"] == "judge_github_primary_v1"
    assert first_judge["schema_version"] == "judge_output_v1"
    assert first_judge["policy_version"] == "verdict_policy_v1"
    assert first_judge["finish_reason"] == "fake_structured_output"
    assert first_judge["refusal_detected"] is False
    assert first_judge["input_tokens"] is not None
    assert first_judge["output_tokens"] is not None
...
def _run_cli(*, database_url: str, namespace: str) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(
        [
            sys.executable,
            "-m",
            "tools.local_db_fake_judge_output_fixture_replay_runner",
            "--database-url",
            database_url,
            "--source-fixture",
        ...
        with engine.connect() as connection:
            return {
                "namespace_judge_output_ready_events": _count(
                    ...
                    """,
                    {"dedupe_prefix": f"local-db-fake-judge-output:{namespace}:judge.output.ready:%"},
                ),
                "namespace_forbidden_downstream_events": _count(
                    ...
                    WHERE event_type IN (
                      ...
                      AND dedupe_key LIKE :dedupe_prefix
                    """,
                    {"dedupe_prefix": f"local-db-fake-judge-output:{namespace}:%"},
                ),
