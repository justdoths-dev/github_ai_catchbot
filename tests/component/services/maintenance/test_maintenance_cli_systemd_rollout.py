...
def _assert_sanitized_context_proof_block(output: str, *, reason_code: str, missing_env_file) -> None:
    payload = json.loads(output)
    assert payload["schema_version"] == "maintenance_systemd_context_proof_report_v1"
    assert payload["status"] == "blocked"
    assert payload["reason_code"] == reason_code
    assert payload["service_name_matches_expected"] is False
    assert payload["service_file_present"] is False
    assert payload["unit_context_matches_expected"] is False
    assert payload["raw_unit_content_omitted"] is True
    assert payload["raw_exec_start_omitted"] is True
    assert payload["raw_paths_omitted"] is True
    assert payload["runtime_env_values_omitted"] is True
    assert str(missing_env_file) not in output
    assert "sentinel-secret" not in output
...
@pytest.mark.asyncio
async def test_context_proof_invalid_target_blocks_before_env_file_or_config_load(
    monkeypatch,
    tmp_path,
    capsys,
) -> None:
    ...
    assert exit_code == 2
    _assert_sanitized_context_proof_block(
        output,
        reason_code="target_not_allowed",
        missing_env_file=missing_env_file,
    )
