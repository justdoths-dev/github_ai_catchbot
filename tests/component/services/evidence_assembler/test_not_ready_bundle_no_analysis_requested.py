...
@pytest.mark.asyncio
async def test_not_ready_bundle_does_not_emit_analysis_requested() -> None:
    repository = FakeRepository()
    trigger_event_id = uuid4()
    artifact_id = uuid4()
    candidate_group_id = uuid4()
    add_candidate(repository, candidate_group_id=candidate_group_id, primary_artifact_id=artifact_id, artifact_type="web_article")
    repository.snapshots[artifact_id] = snapshot(artifact_id, snapshot_type="web_article", status="access_denied")
