...
pytestmark = pytest.mark.skipif(
    ...
    reason="LOCAL_TEST_DATABASE_URL is required for the DB-backed X-primary GitHub-supporting reroot e2e test",
)
...
class SeedRerootIds:
    ...
    x_artifact_id: UUID
    github_artifact_id: UUID
    x_enrich_requested_event_id: UUID
    github_enrich_requested_event_id: UUID
    x_post_id: str
    x_canonical_id: str
    x_canonical_url: str
...
@pytest.mark.asyncio
async def test_db_backed_x_primary_github_supporting_reroot_creates_notifier_dry_run_records() -> None:
    database_url = _local_test_database_url()
    engine = create_async_engine(database_url, future=True)
    try:
        async with AsyncSession(engine, expire_on_commit=False) as session:
            ids = await _seed_x_primary_github_supporting_case(session)
            ...
            assert candidate_members_before == [
                ...
                {
                    "candidate_group_id": ids.candidate_group_id,
                    "artifact_id": ids.github_artifact_id,
            ...
            github_enrich_events = await _events(
                session,
                event_type="artifact.enrich.requested.v1",
                aggregate_id=ids.github_artifact_id,
            )
            ...
            assert github_enrich_events[0]["event_id"] == ids.github_enrich_requested_event_id
            ...
            assert github_enrich_events[0]["payload_json"] == {
                "candidate_group_id": str(ids.candidate_group_id),
                "artifact_id": str(ids.github_artifact_id),
            ...
            await _move_event_to_front(session, ids.github_enrich_requested_event_id)
            await session.commit()
            ...
            assert github_fields == {
                "job_id": str(ids.github_enrich_requested_event_id),
                "stage_name": "enrich_github",
                "root_object_type": "artifact",
                "root_object_id": str(ids.github_artifact_id),
                "idempotency_key": github_enrich_events[0]["dedupe_key"],
                "pipeline_run_id": "",
                "not_before": "",
                "trigger_event_id": str(ids.github_enrich_requested_event_id),
            }
            ...
            github_runs = await _github_artifact_enrichment_runs(session, ids.github_artifact_id)
            assert len(github_runs) == 1
            assert github_runs[0]["provider"] == "github"
            assert github_runs[0]["status"] in {"ready", "partial_ready"}

            github_snapshots = await _artifact_snapshots_for_artifact(session, ids.github_artifact_id)
            ...
            assert await _artifact_current_snapshot(session, ids.github_artifact_id) == {
            ...
            assert reroot_events == [
                {
                    ...
                    "to_artifact_id": ids.github_artifact_id,
                    "reason_code": "x_post_discovered_github_repo_supporting_reroot",
                    "trigger_snapshot_id": github_snapshot_id,
                }
            ]
            assert await _candidate_group_current_primary(session, ids.candidate_group_id) == ids.github_artifact_id
            assert await _candidate_group_members(session, ids.candidate_group_id) == candidate_members_before
            ...
            assert bundle_details == {
                ...
                "current_primary_artifact_id": ids.github_artifact_id,
                "reroot_count": 1,
            ...
            before_notifier_snapshot = await _upstream_boundary_snapshot(
                ...
                artifact_ids=[ids.x_artifact_id, ids.github_artifact_id],
                candidate_group_id=ids.candidate_group_id,
                bundle_id=bundle_id,
...
async def _seed_x_primary_github_supporting_case(session: AsyncSession) -> SeedRerootIds:
    source_message_id = uuid4()
    candidate_group_id = uuid4()
    x_artifact_id = uuid4()
    github_artifact_id = uuid4()
    x_event_id = uuid4()
    github_event_id = uuid4()
    suffix = uuid4().hex
    now = datetime.now(timezone.utc)
    x_post_id = str(1_890_000_000_000_000_000 + int(suffix[:10], 16))
    owner = "example"
    repo = f"x-reroot-supporting-{suffix[:12]}"
    ...
    source_text = (
        "X primary with already-known supporting GitHub repo for reroot E2E: "
        f"{x_canonical_url} {github_canonical_url}"
    )
    raw_message_json = {
        "local_test": True,
    ...
    await session.execute(
        ...
        ),
        {
            ...
            "logical_post_key": f"db-x-primary-github-supporting-reroot-e2e:{suffix}",
            "posted_at": now,
    ...
    await session.execute(
        sa.text(
            ...
            INSERT INTO artifact_registry (
              ...
              (
                CAST(:github_artifact_id AS uuid), 'github_repo'::artifact_type_enum,
                :github_canonical_id, :github_canonical_url, 'github.com',
                CAST(:github_artifact_key_json AS jsonb)
              )
            """
        ),
        {
            ...
            "github_artifact_id": str(github_artifact_id),
            "github_canonical_id": github_canonical_id,
            "github_canonical_url": github_canonical_url,
            "github_artifact_key_json": _jsonb({"owner": owner, "repo": repo}),
        },
    )
    for artifact_id, observed_url, source_kind, canonical_url, classification, context_path in [
        ...
        (github_artifact_id, github_canonical_url, "regex", github_canonical_url, "github_repo", "urls[1]"),
    ]:
        await session.execute(
            sa.text(
                """
    ...
    await session.execute(
        ...
        ),
        {
            ...
            "source_message_id": str(source_message_id),
            "x_artifact_id": str(x_artifact_id),
            "normalizer_version": "db-x-primary-github-supporting-reroot-e2e-test-v1",
            "dedupe_subject_key": f"db-x-primary-github-supporting-reroot-e2e:{suffix}",
        },
    )
    await session.execute(
        sa.text(
            ...
            INSERT INTO candidate_group_members (
              ...
              (CAST(:candidate_group_id AS uuid), CAST(:github_artifact_id AS uuid), 'supporting', 1)
            """
        ),
        {
            "candidate_group_id": str(candidate_group_id),
    ...
    await _insert_enrich_requested_event(
        ...
        provider_route="x",
        dedupe_key=f"db-x-primary-github-supporting-reroot-e2e:{suffix}:x.enrich.requested",
    )
    await _insert_enrich_requested_event(
        session,
        ...
        source_message_id=source_message_id,
        artifact_type="github_repo",
        canonical_id=github_canonical_id,
        provider_route="github",
        dedupe_key=f"db-x-primary-github-supporting-reroot-e2e:{suffix}:github.enrich.requested",
    )
    return SeedRerootIds(
        source_message_id=source_message_id,
        candidate_group_id=candidate_group_id,
        x_artifact_id=x_artifact_id,
        github_artifact_id=github_artifact_id,
        x_enrich_requested_event_id=x_event_id,
        github_enrich_requested_event_id=github_event_id,
        x_post_id=x_post_id,
        x_canonical_id=x_canonical_id,
        x_canonical_url=x_canonical_url,
        github_canonical_id=github_canonical_id,
