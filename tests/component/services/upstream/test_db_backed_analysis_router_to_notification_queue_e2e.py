...
pytestmark = pytest.mark.skipif(
    ...
    reason="LOCAL_TEST_DATABASE_URL is required for the DB-backed upstream analysis-to-notification queue test",
)
...
class SeedIds:
    ...
    bundle_id: UUID
    analysis_requested_event_id: UUID
...
class RecordingOpenAIClient:
    ...
    async def create_structured_response(self, **kwargs: Any) -> dict[str, Any]:
        ...
        return {
            "id": "local-db-upstream-e2e-fake-response",
            "status": "completed",
...
@pytest.mark.asyncio
async def test_db_backed_analysis_requested_routes_to_notification_queue_with_fake_judge_openai() -> None:
    database_url = _local_test_database_url()
    engine = create_async_engine(database_url, future=True)
    try:
        async with AsyncSession(engine, expire_on_commit=False) as session:
            ids = await _seed_analysis_requested_case(session, ready_for_analysis=True)
            ...
            await router.handle_trigger_event(ids.analysis_requested_event_id)
            await router.handle_trigger_event(ids.analysis_requested_event_id)
            await session.commit()

            judge_runs = await _judge_runs_for_bundle(session, ids.bundle_id)
...
@pytest.mark.asyncio
async def test_db_backed_not_ready_analysis_request_does_not_create_judge_call() -> None:
    database_url = _local_test_database_url()
    engine = create_async_engine(database_url, future=True)
    try:
        async with AsyncSession(engine, expire_on_commit=False) as session:
            ids = await _seed_analysis_requested_case(session, ready_for_analysis=False)
            await session.commit()
            ...
            assert await _judge_runs_for_bundle(session, ids.bundle_id) == []
            assert await _judge_call_events_for_bundle(session, ids.bundle_id) == []
            refresh_events = await _events(
                session,
                event_type="candidate.bundle.refresh.v1",
                aggregate_id=ids.candidate_group_id,
            )
            assert len(refresh_events) == 1
            assert refresh_events[0]["payload_json"] == {
                "candidate_group_id": str(ids.candidate_group_id),
                "trigger_kind": "analysis_router_recheck",
                "trigger_object_type": "bundle",
                "trigger_object_id": str(ids.bundle_id),
                "refresh_reason": "bundle_not_ready",
            }
    finally:
        await engine.dispose()


async def _seed_analysis_requested_case(session: AsyncSession, *, ready_for_analysis: bool) -> SeedIds:
    ...
    canonical_id = f"github:local-db-upstream-e2e:{suffix}"
    canonical_url = "https://github.com/example/local-db-upstream-e2e"
    ...
    await session.execute(
        ...
        ),
        {
            ...
            "chat_id": 9100000000 + int(suffix[:8], 16),
            "message_id": int(suffix[8:16], 16),
            "logical_post_key": f"db-upstream-e2e:{suffix}",
            "posted_at": now,
            "text_body": "Repository signal for local DB upstream analysis notification queue test.",
    ...
    await session.execute(
        ...
        ),
        {
            ...
            "artifact_id": str(artifact_id),
            "fetched_at": now,
            "content_anchor": f"local-db-upstream-e2e:{suffix}",
            "normalized_projection": _jsonb(
                {
                    "title": "local-db-upstream-e2e",
    ...
    await session.execute(
        ...
        ),
        {
            ...
            "source_message_id": str(source_message_id),
            "artifact_id": str(artifact_id),
            "normalizer_version": "db-upstream-e2e-test-v1",
            "dedupe_subject_key": f"db-upstream-e2e:{suffix}",
        },
    )
    await session.execute(
        sa.text(
    ...
    await session.execute(
        sa.text(
            ...
            INSERT INTO candidate_evidence_bundles (
                ...
                CAST(:artifact_id AS uuid), CAST(:artifact_id AS uuid),
                'db_upstream_e2e_bundle_v1', :bundle_input_hash,
                CAST(:primary_summary AS jsonb), CAST(:supporting AS jsonb),
                CAST(:discovered AS jsonb), CAST(:limitations AS jsonb),
                :ready_for_analysis, 'test'
            )
            """
        ),
        {
            ...
            "candidate_group_id": str(candidate_group_id),
            "artifact_id": str(artifact_id),
            "bundle_input_hash": f"db-upstream-e2e:{suffix}",
            "primary_summary": _jsonb(
                {
                    "artifact_type": "github_repo",
                    "canonical_id": canonical_id,
                    "canonical_url": canonical_url,
                    "repo_full_name": "example/local-db-upstream-e2e",
                    "summary": "Deterministic GitHub repository summary for local DB upstream E2E.",
                }
            ),
            "supporting": _jsonb([]),
            "discovered": _jsonb([]),
            "limitations": _jsonb(["local DB component fixture"]),
    ...
    await session.execute(
        sa.text(
            ...
            INSERT INTO event_outbox (
                ...
                CAST(:event_id AS uuid), 'analysis.requested.v1', 'candidate_group',
                CAST(:candidate_group_id AS uuid), :dedupe_key,
                CAST(:payload_json AS jsonb), 'published'::outbox_status_enum, :created_at
            )
            """
        ),
        {
            "event_id": str(analysis_requested_event_id),
            "candidate_group_id": str(candidate_group_id),
            "dedupe_key": f"db-upstream-e2e:{suffix}:analysis.requested",
            "payload_json": _jsonb(
                {
                    "candidate_group_id": str(candidate_group_id),
                    "bundle_id": str(bundle_id),
...
async def _judge_call_events_for_bundle(session: AsyncSession, bundle_id: UUID) -> list[dict[str, Any]]:
    result = await session.execute(
        sa.text(
            """
            SELECT event_id, dedupe_key, payload_json, aggregate_type, aggregate_id, status
