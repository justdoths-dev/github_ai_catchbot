...
class FakeRepository:
    def __init__(self, rows: list[OutboxEventRow]) -> None:
        ...
        self.published_at_by_event_id: dict[UUID, datetime] = {}
    ...
    async def mark_published(self, *, event_id: UUID, published_at: datetime | None = None) -> None:
        ...
        self.published_at_by_event_id[event_id] = published_at or datetime.now(timezone.utc)
...
class FakePublisher:
    def __init__(self) -> None:
        self.calls: list[tuple[QueueRoute, RedisQueuedMessage]] = []
...
@pytest.mark.asyncio
async def test_publish_marks_outbox_published_and_writes_job_attempt() -> None:
    row = OutboxEventRow(
        event_id=uuid4(),
    ...
    assert processed == 1
    assert repository.published_event_ids == [row.event_id]
    assert row.event_id in repository.published_at_by_event_id
    assert repository.failed_event_ids == []
