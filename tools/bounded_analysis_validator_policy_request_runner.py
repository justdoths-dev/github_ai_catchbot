...
def build_parser() -> argparse.ArgumentParser:
    parser = JsonOnlyArgumentParser(
        description=(
            "Validate one exact q.analysis.validate judge.output.ready.v1 handoff "
            "and publish one analysis.policy.apply.v1 request."
        ),
        add_help=False,
    )
    ...
    parser.add_argument("--allow-analysis-validator", action="store_true")
...
def run(
    ...
    *,
    ...
    judge_output_suffix = _parse_optional_uuid_suffix(
        args.judge_output_suffix,
        "invalid_judge_output_suffix",
    )
    judge_run_suffix = _parse_optional_uuid_suffix(
        args.judge_run_suffix,
        "invalid_judge_run_suffix",
    )
    for parsed in (redis_message_suffix, trigger_event_suffix, judge_output_suffix, judge_run_suffix):
    ...
    result = run_bounded_analysis_validator_policy_request_sync(
        BoundedAnalysisValidatorPolicyRequestConfig(
            ...
            allow_analysis_validator=bool(args.allow_analysis_validator),
            redis_message_suffix=redis_message_suffix,
            trigger_event_suffix=trigger_event_suffix,
            judge_output_suffix=judge_output_suffix,
            judge_run_suffix=judge_run_suffix,
