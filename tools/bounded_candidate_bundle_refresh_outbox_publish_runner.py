...
def build_parser() -> argparse.ArgumentParser:
    parser = JsonOnlyArgumentParser(
        description="Publish exactly one pending candidate.bundle.refresh.v1 outbox row to q.candidate.bundle.",
        add_help=False,
    )
...
def run(
    ...
    *,
    ...
    if isinstance(candidate_group_id, dict):
        return RunnerResult(exit_code=1, report=candidate_group_id)
    ...
    result = run_bounded_candidate_bundle_refresh_outbox_publish_sync(
        BoundedCandidateBundleRefreshOutboxPublishConfig(
