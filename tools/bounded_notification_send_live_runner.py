...
def build_parser() -> argparse.ArgumentParser:
    parser = JsonOnlyArgumentParser(
        description="Preview or execute one exact gated q.notification.send live delivery.",
        add_help=False,
    )
    ...
    parser.add_argument("--allow-maintenance-publish", action="store_true")
    ...
    parser.add_argument("--allow-telegram-transport", action="store_true")
    parser.add_argument("--allow-telegram-send", action="store_true")
    return parser
...
def run(
    ...
    *,
    runtime_config_loader=load_bounded_notification_send_live_runtime_config,
    runtime_builder: BoundedNotificationSendLiveRuntimeBuilder | None = None,
) -> RunnerResult:
    result = run_bounded_notification_send_live_sync(
        BoundedNotificationSendLiveConfig(
            mode=str(args.mode),
            ...
            allow_maintenance_publish=bool(args.allow_maintenance_publish),
            ...
            allow_telegram_transport=bool(args.allow_telegram_transport),
            allow_telegram_send=bool(args.allow_telegram_send),
