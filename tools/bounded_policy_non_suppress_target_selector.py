...
def build_parser() -> argparse.ArgumentParser:
    parser = JsonOnlyArgumentParser(
        description=(
            "Read q.analysis.policy and PostgreSQL to select existing exact targets whose deterministic "
            "policy preview is non-suppress. Does not run the policy engine or publish Redis."
        ),
        add_help=False,
    )
