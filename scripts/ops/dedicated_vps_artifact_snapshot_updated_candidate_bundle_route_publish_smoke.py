...
SCHEMA_VERSION = "1.0"
SCRIPT_NAME = "dedicated_vps_artifact_snapshot_updated_candidate_bundle_route_publish_smoke"
REPORT_TYPE = "artifact_snapshot_updated_candidate_bundle_route_publish_smoke_v1"
...
STATUS_READY = "artifact_snapshot_updated_candidate_bundle_route_publish_smoke_ready"
STATUS_PUBLISHED = "artifact_snapshot_updated_candidate_bundle_route_publish_smoke_published"
STATUS_MISSING_APPROVAL = (
    "blocked_artifact_snapshot_updated_candidate_bundle_route_publish_smoke_missing_approval"
)
STATUS_NO_PENDING = (
    "blocked_artifact_snapshot_updated_candidate_bundle_route_publish_smoke_no_pending_event"
)
STATUS_INVALID_EVENT = (
    "blocked_artifact_snapshot_updated_candidate_bundle_route_publish_smoke_invalid_event"
)
STATUS_REDIS_PUBLISH_FAILED = (
    "blocked_artifact_snapshot_updated_candidate_bundle_route_publish_smoke_redis_publish_failed"
)
STATUS_DB_UPDATE_FAILED = (
    "blocked_artifact_snapshot_updated_candidate_bundle_route_publish_smoke_db_update_failed"
)
STATUS_FORBIDDEN_SIDE_EFFECT = "blocked_forbidden_side_effect_detected"
STATUS_NOT_READY = (
    "blocked_artifact_snapshot_updated_candidate_bundle_route_publish_smoke_not_ready"
)
...
SELECT_PENDING_SNAPSHOT_UPDATED_EVENTS_QUERY = """
SELECT
    event_id,
    event_type,
    aggregate_type,
...
WHERE status = 'pending'::outbox_status_enum
  AND event_type = 'artifact.snapshot.updated.v1'
  AND aggregate_type = 'artifact'
ORDER BY created_at DESC, event_id DESC
LIMIT :limit
"""
COUNT_ARTIFACT_SNAPSHOT_QUERY = """
SELECT COUNT(*)
...
class PublishApprovals:
    artifact_snapshot_updated_candidate_bundle_route_publish: bool
    redis_publish: bool
    ...
    def all_granted(self) -> bool:
        return (
            self.artifact_snapshot_updated_candidate_bundle_route_publish
    ...
    def missing_checks(self) -> list[str]:
        checks: list[str] = []
        if not self.artifact_snapshot_updated_candidate_bundle_route_publish:
            checks.append(
                "approval.artifact_snapshot_updated_candidate_bundle_route_publish"
            )
...
class SnapshotUpdatedOutboxRepository(OutboxRelayRepository):
    async def fetch_recent_pending_snapshot_updated_events(
        self,
        *,
        limit: int,
    ) -> list[OutboxEventRow]:
        result = await self._session.execute(
            _sql(SELECT_PENDING_SNAPSHOT_UPDATED_EVENTS_QUERY),
            {"limit": limit},
        )
    ...
    async def x_snapshot_exists(self, *, artifact_id: str, snapshot_id: str) -> bool:
        artifact_snapshot_count = _safe_count(
            await _scalar(
                await self._session.execute(
                    _sql(COUNT_ARTIFACT_SNAPSHOT_QUERY),
                    {
        ...
        if artifact_snapshot_count <= 0:
            return False
        x_post_count = _safe_count(
            await _scalar(
                await self._session.execute(
                    _sql(COUNT_X_POST_SNAPSHOT_QUERY),
                    {"snapshot_id": snapshot_id},
                )
            )
        )
        return x_post_count > 0
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Bounded artifact.snapshot.updated.v1 candidate-bundle route publish smoke. "
            "Default mode validates one pending X snapshot-updated event and plans the "
            "thin Redis publish without mutating Redis or PostgreSQL."
        )
    )
    ...
    parser.add_argument(
        "--approved-artifact-snapshot-updated-candidate-bundle-route-publish",
        action="store_true",
    )
...
def _base_report() -> dict[str, Any]:
    return {
        ...
        "snapshot_updated_pending_event_found_bucket": "zero",
        "snapshot_updated_event_valid_bucket": "zero",
        "provider_route_bucket": "zero",
...
async def _event_shape_is_consistent(
    *,
    repository: SnapshotUpdatedOutboxRepository,
    row: OutboxEventRow,
) -> tuple[bool, str | None]:
    ...
    if not snapshot_id:
        return False, "payload.snapshot_id"
    if artifact_id != str(row.aggregate_id):
        return False, "aggregate.artifact_id"
    if provider != EXPECTED_PROVIDER_ROUTE:
        return False, "payload.provider"
    if provider_route is not None and provider_route != EXPECTED_PROVIDER_ROUTE:
        return False, "payload.provider_route"
    if not await repository.x_snapshot_exists(
        artifact_id=artifact_id,
        snapshot_id=snapshot_id,
    ):
        return False, "snapshot.x_post"
    return True, None
...
async def _select_target(
    *,
    ...
    rows = await repository.fetch_recent_pending_snapshot_updated_events(
        limit=pending_scan_limit,
    )
    raw_values = _raw_values_from_rows(rows)
    report["snapshot_updated_pending_event_found_bucket"] = _bucket_count(len(rows))

    provider_x_count = 0
    valid_event_count = 0
    stream_name_valid_count = 0
    thin_payload_valid_count = 0
    selected: SelectedTarget | None = None
    invalid_checks: list[str] = []

    for row in rows:
        payload_provider = _payload_str(row.payload_json, "provider")
        payload_provider_route = _payload_str(row.payload_json, "provider_route")
        if payload_provider != EXPECTED_PROVIDER_ROUTE:
            invalid_checks.append("payload.provider")
            continue
        if payload_provider_route is not None and payload_provider_route != EXPECTED_PROVIDER_ROUTE:
            invalid_checks.append("payload.provider_route")
            continue
        provider_x_count += 1
        ...
        if not shape_ready:
            invalid_checks.append(shape_check or "event.shape")
            continue
        valid_event_count += 1
        ...
        selected = SelectedTarget(row=row, route=route, message=message)

    report["provider_route_bucket"] = _bucket_count(provider_x_count)
    report["snapshot_updated_event_valid_bucket"] = _bucket_count(valid_event_count)
    report["redis_stream_name_bucket"] = _bucket_count(stream_name_valid_count)
    report["redis_thin_payload_valid_bucket"] = _bucket_count(thin_payload_valid_count)
    ...
    if valid_event_count > 1:
        _set_status(report, STATUS_INVALID_EVENT, "event_outbox.multiple_valid_events")
        return None, raw_values, False
    if selected is None:
        _set_status(report, STATUS_INVALID_EVENT)
        for check in invalid_checks:
    ...
    return selected, raw_values, True
...
async def _publish_selected_target(
    *,
    ...
    try:
        ...
        await session.rollback()
        _set_status(report, STATUS_DB_UPDATE_FAILED, "database.update")
        return ScriptResult(exit_code=1, report=report)
...
async def generate_report_async(
    *,
    ...
    try:
        ...
        repository = SnapshotUpdatedOutboxRepository(session)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = generate_report(
        ...
        approvals=PublishApprovals(
            artifact_snapshot_updated_candidate_bundle_route_publish=(
                args.approved_artifact_snapshot_updated_candidate_bundle_route_publish
            ),
            redis_publish=args.approved_redis_publish,
            event_outbox_update=args.approved_event_outbox_update,
