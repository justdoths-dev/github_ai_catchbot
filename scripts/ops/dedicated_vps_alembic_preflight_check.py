...
REPORT_TYPE = "dedicated_vps_alembic_preflight_check_v1"
CHECKED_FILE = "ops/pipeline/runbooks/dedicated_vps_alembic_preflight.md"
...
REQUIRED_SECTIONS = (
    RequiredSection("# Dedicated VPS redacted Alembic migration preflight"),
    RequiredSection("## Scope"),
    RequiredSection("## Source-of-truth handling"),
    RequiredSection("## Current prerequisite state"),
    RequiredSection("## Allowed preflight checks"),
...
REQUIRED_MARKERS = (
    ...
    _marker("secret_values_printed_false", "Require secret_values_printed=false evidence.", "secret_values_printed=false"),
    _marker("alembic_upgrade_run_false", "Require alembic_upgrade_run=false evidence.", "alembic_upgrade_run=false"),
    _marker("alembic_stamp_run_false", "Require alembic_stamp_run=false evidence.", "alembic_stamp_run=false"),
    _marker("alembic_revision_run_false", "Require alembic_revision_run=false evidence.", "alembic_revision_run=false"),
    _marker(
        "separate_approval",
        "Require separate approval before Alembic current.",
        "Run only after separate approval",
    ),
    _marker("alembic_current", "Document the read-only Alembic current command.", "python -m alembic current"),
    _marker("no_upgrade", "Explicitly forbid Alembic upgrade.", "Do not run `alembic upgrade`"),
...
FORBIDDEN_PATTERNS = (
    ...
    ForbiddenPattern("alembic_upgrade", r"\balembic\s+upgrade\b", "Do not authorize Alembic upgrade."),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Validate the dedicated VPS Alembic preflight runbook from local repository text only. "
        ...
        failures.append(
            {
                ...
                "path": CHECKED_FILE,
                "message": "Dedicated VPS Alembic preflight runbook is missing.",
            }
        )
    else:
