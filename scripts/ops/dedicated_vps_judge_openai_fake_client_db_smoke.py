...
from src.services.judge_openai.models import JudgeCallJob
from src.services.judge_openai.prompt_library import PromptLibrary, UnsupportedJudgeProfileError
from src.services.judge_openai.repositories import JudgeOpenAIRepository
from src.services.judge_openai.service import JudgeOpenAIService


SCHEMA_VERSION = "1.0"
SCRIPT_NAME = "dedicated_vps_judge_openai_fake_client_db_smoke"
REPORT_TYPE = "judge_openai_fake_client_db_smoke_v1"
...
FAKE_REFUSAL_TEXT = "fake judge-openai db smoke response; not a live OpenAI judgment"

STATUS_PREFLIGHT_PASSED = "judge_openai_fake_client_db_smoke_preflight_passed"
STATUS_DB_WRITE_PASSED = "judge_openai_fake_client_db_smoke_approved_db_write_passed"
STATUS_NOT_READY = "blocked_judge_openai_fake_client_db_smoke_not_ready"
STATUS_APPROVAL_MISSING = "blocked_judge_openai_fake_client_db_smoke_missing_approval"
STATUS_NO_CANDIDATE = "blocked_judge_openai_fake_client_db_smoke_no_candidate"
STATUS_AMBIGUOUS_CANDIDATE = "blocked_judge_openai_fake_client_db_smoke_ambiguous_candidate"
STATUS_INVALID_CANDIDATE = "blocked_judge_openai_fake_client_db_smoke_invalid_candidate"
STATUS_DUPLICATE_OUTPUT = "blocked_judge_openai_fake_client_db_smoke_existing_output"
STATUS_DUPLICATE_READY_OUTBOX = "blocked_judge_openai_fake_client_db_smoke_existing_ready_outbox"
STATUS_NON_PENDING_RUN = "blocked_judge_openai_fake_client_db_smoke_non_pending_run"
STATUS_MISSING_BUNDLE = "blocked_judge_openai_fake_client_db_smoke_missing_or_invalid_bundle"
STATUS_FORBIDDEN_SIDE_EFFECT = "blocked_judge_openai_fake_client_db_smoke_forbidden_side_effect"
STATUS_WRITE_FAILED = "blocked_judge_openai_fake_client_db_smoke_write_failed"
STATUS_RAW_VALUE_EMISSION = "blocked_judge_openai_fake_client_db_smoke_raw_value_emission"
...
class FakeRefusalOpenAIClient:
    def __init__(self) -> None:
        self.calls = 0

    async def create_structured_response(self, **_kwargs: Any) -> dict[str, Any]:
        self.calls += 1
        return {
            "status": "completed",
            "output": [
                {
                    "type": "message",
                    "content": [{"type": "refusal", "refusal": FAKE_REFUSAL_TEXT}],
                }
            ],
            "usage": {
                "input_tokens": 0,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Approval-gated fake-client DB smoke for a single pending "
    ...
    parser.add_argument("--approve-fake-openai", action="store_true")
...
async def _select_candidate_from_redis(
    *,
    ...
    report["candidate_judge_message_found"] = entry_count == 1
...
async def _load_event_row(
    *,
    ...
    report["event_outbox_rehydrated"] = row is not None
...
async def _inspect_job_preconditions(
    *,
    ...
    report["judge_run_pending"] = judge_run.status == "pending"
    ...
    report["bundle_ready_for_judge"] = bundle_ready
    ...
    if analysis_count or notification_count:
...
def _config_for_fake_smoke(database_url: str, redis_url: str) -> JudgeOpenAIConfig:
    return JudgeOpenAIConfig(
        ...
        batch_size=1,
        block_ms=1,
        openai_api_key="fake-client-only",
        openai_project=None,
...
async def _run_approved_fake_db_write(
    *,
    report: dict[str, Any],
    session: AsyncSessionLike,
    repository: JudgeOpenAIRepository,
    ...
    fake_client = FakeRefusalOpenAIClient()
    service = JudgeOpenAIService(
        _config_for_fake_smoke(database_url, redis_url),
        repository=repository,
        openai_client=fake_client,
    )
    await service.handle_job(job)
    report["fake_openai_used"] = fake_client.calls == 1
    ...
    report["judge_run_updated_bucket"] = "one" if _run_succeeded_with_refusal(run_state) else "zero"


def _run_succeeded_with_refusal(row: Mapping[str, Any] | None) -> bool:
    if row is None:
        return False
    return str(row.get("status")) == "succeeded" and bool(row.get("refusal_detected"))


def _approved_execution_succeeded(report: Mapping[str, Any]) -> bool:
    return bool(
        report["fake_openai_used"]
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = generate_report(
        ...
        approve_fake_openai=args.approve_fake_openai,
    )
