...
SCHEMA_VERSION = "1.0"
SCRIPT_NAME = "dedicated_vps_judge_call_requested_route_publish_smoke"
REPORT_TYPE = "judge_call_requested_route_publish_smoke_v1"
...
STATUS_READY = "judge_call_requested_route_publish_smoke_ready"
STATUS_PUBLISHED = "judge_call_requested_route_publish_smoke_published"
STATUS_MISSING_APPROVAL = "blocked_judge_call_requested_route_publish_smoke_missing_approval"
STATUS_NO_PENDING = "blocked_judge_call_requested_route_publish_smoke_no_pending_judge_call_request"
STATUS_INVALID_EVENT = "blocked_judge_call_requested_route_publish_smoke_invalid_event"
STATUS_INVALID_JUDGE_RUN = "blocked_judge_call_requested_route_publish_smoke_invalid_judge_run"
STATUS_INVALID_BUNDLE = "blocked_judge_call_requested_route_publish_smoke_invalid_bundle"
STATUS_ROUTE_MISMATCH = "blocked_judge_call_requested_route_publish_smoke_route_mismatch"
STATUS_REDIS_PUBLISH_FAILED = (
    "blocked_judge_call_requested_route_publish_smoke_redis_publish_failed"
)
STATUS_DB_UPDATE_FAILED = "blocked_judge_call_requested_route_publish_smoke_db_update_failed"
STATUS_FORBIDDEN_SIDE_EFFECT = "blocked_forbidden_side_effect_detected"
STATUS_RAW_VALUE_EMISSION = "blocked_judge_call_requested_route_publish_smoke_raw_value_emission"
STATUS_NOT_READY = "blocked_judge_call_requested_route_publish_smoke_not_ready"
...
SELECT_PENDING_JUDGE_CALL_REQUESTED_EVENTS_QUERY = """
SELECT
    event_id,
    event_type,
    aggregate_type,
...
class PublishApprovals:
    ...
    def missing_checks(self) -> list[str]:
        checks: list[str] = []
        if not self.judge_call_route_publish:
            checks.append("approval.judge_call_route_publish")
...
class JudgeCallRequestedOutboxRepository(OutboxRelayRepository):
    async def fetch_recent_pending_judge_call_requested_events(
        self,
        *,
        limit: int,
    ) -> list[OutboxEventRow]:
        result = await self._session.execute(
            _sql(SELECT_PENDING_JUDGE_CALL_REQUESTED_EVENTS_QUERY),
            {"limit": limit},
        )
    ...
    async def load_judge_run_state(
        self,
        judge_run_id: UUID,
    ) -> JudgeRunState | None:
        result = await self._session.execute(
            _sql(SELECT_JUDGE_RUN_STATE_QUERY),
    ...
    async def load_bundle_state(self, bundle_id: UUID) -> BundleState | None:
        result = await self._session.execute(
            _sql(SELECT_BUNDLE_READY_QUERY),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Bounded judge.call.requested.v1 route publish smoke. Default mode "
            "validates one pending judge call request and the intended thin Redis "
    ...
    parser.add_argument(
        "--approved-judge-call-route-publish",
        action="store_true",
    )
...
def _base_report() -> dict[str, Any]:
    return {
        ...
        "judge_call_requested_pending_event_found_bucket": "zero",
        "judge_call_requested_event_valid_bucket": "zero",
...
async def _validate_judge_call_requested_event(
    *,
    report: dict[str, Any],
    repository: JudgeCallRequestedOutboxRepository,
    row: OutboxEventRow,
) -> tuple[bool, str]:
    ...
    if missing_payload_fields:
        return False, f"payload.{missing_payload_fields[0]}"
    if judge_run_id is None:
        return False, "payload.judge_run_id"
    if bundle_id is None:
        return False, "payload.bundle_id"
    if row.aggregate_id != judge_run_id:
        return False, "aggregate.judge_run_id"
    report["judge_call_requested_event_valid_bucket"] = "one"

    judge_run = await repository.load_judge_run_state(judge_run_id)
    ...
    if judge_run.schema_version != EXPECTED_SCHEMA_VERSION:
        return False, "judge_run.schema_version"
    if judge_run.policy_version != EXPECTED_POLICY_VERSION:
        return False, "judge_run.policy_version"
...
async def _select_target(
    *,
    ...
    rows = await repository.fetch_recent_pending_judge_call_requested_events(
        limit=pending_scan_limit
    )
    raw_values = _raw_values_from_rows(rows)
    report["judge_call_requested_pending_event_found_bucket"] = _bucket_count(len(rows))
    if not rows:
        _set_status(report, STATUS_NO_PENDING, "event_outbox.no_pending_judge_call_request")
    ...
    row = rows[0]
    valid, check = await _validate_judge_call_requested_event(
        report=report,
...
async def generate_report_async(
    *,
    ...
    try:
        ...
        repository = JudgeCallRequestedOutboxRepository(session)
...
def _approvals_from_args(args: argparse.Namespace) -> PublishApprovals:
    return PublishApprovals(
        judge_call_route_publish=bool(args.approved_judge_call_route_publish),
