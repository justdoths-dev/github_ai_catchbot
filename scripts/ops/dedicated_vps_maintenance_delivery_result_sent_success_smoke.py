...
SCHEMA_VERSION = "1.0"
SCRIPT_NAME = "dedicated_vps_maintenance_delivery_result_sent_success_smoke"
REPORT_TYPE = "maintenance_delivery_result_sent_success_smoke_v1"
...
STATUS_DEFAULT_PASSED = "maintenance_delivery_result_sent_success_smoke_default_passed"
STATUS_DB_PREFLIGHT_PASSED = "maintenance_delivery_result_sent_success_smoke_db_read_preflight_passed"
STATUS_DB_WRITE_PASSED = "maintenance_delivery_result_sent_success_smoke_approved_db_write_passed"
STATUS_NOT_APPROVED = "blocked_maintenance_delivery_result_sent_success_smoke_not_approved"
STATUS_DB_READ_FAILED = "blocked_maintenance_delivery_result_sent_success_smoke_db_read_failed"
STATUS_NO_TARGET = "blocked_maintenance_delivery_result_sent_success_smoke_no_clean_delivery_result"
STATUS_MULTIPLE_TARGETS = "blocked_maintenance_delivery_result_sent_success_smoke_multiple_delivery_results"
STATUS_WRONG_DELIVERY_RESULT = "blocked_maintenance_delivery_result_sent_success_smoke_wrong_delivery_result"
STATUS_EXISTING_MARKER = "blocked_maintenance_delivery_result_sent_success_smoke_existing_marker"
STATUS_FORBIDDEN_EXISTING_EFFECT = "blocked_maintenance_delivery_result_sent_success_smoke_forbidden_existing_effect"
STATUS_WRITE_FAILED = "blocked_maintenance_delivery_result_sent_success_smoke_write_failed"
STATUS_RAW_VALUE_EMISSION = "blocked_maintenance_delivery_result_sent_success_smoke_raw_value_emission"

UUID_PATTERN = (
    "^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$"
)

SELECT_SENT_DELIVERY_RESULT_TARGETS_QUERY = f"""
...
WHERE ce.payload_notification_plan_id = np.notification_plan_id::text
  AND ndr.delivery_status = 'sent'::notification_status_enum
...
COUNT_MAINTENANCE_SENT_SUCCESS_MARKER_FOR_TARGET_QUERY = """
SELECT COUNT(*)
FROM job_attempts
...
class TargetCounts:
    ...
    judge_outputs: int
    candidates: int
    maintenance_sent_success_markers: int
    retry_intents: int
    dead_letters: int
    replay_requests: int
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Approval-gated maintenance smoke for an existing restricted Telegram "
            "send canary notification.delivery.result.v1(sent). Default mode reads "
            "no runtime env, DB, Redis, Telegram token, OpenAI key, or transport."
        )
    )
...
def _base_report(*, approve_db_read: bool, approve_db_write: bool) -> dict[str, Any]:
    return {
        ...
        "target_notification_delivery_status_bucket": "zero",
        ...
        "existing_maintenance_sent_success_marker_bucket": "zero",
...
async def _select_target(
    *,
    ...
    rows = base._rows(await base._execute(session, SELECT_SENT_DELIVERY_RESULT_TARGETS_QUERY))
    ...
    report["target_notification_delivery_status_bucket"] = target.delivery_status or "zero"
    ...
    decision = classify_delivery_result_sent_success(delivery_status=target.delivery_status)
    report["maintenance_classification_bucket"] = decision.maintenance_classification
    if decision.action != "mark_terminal_success":
    ...
    if counts.delivery_result_outbox != 1:
        _set_status(report, STATUS_MULTIPLE_TARGETS, "target.delivery_result_outbox_not_exactly_one")
        return None
    if counts.notification_plans != 1:
        _set_status(report, STATUS_DB_READ_FAILED, "notification_plan.count")
        return None
    if counts.analyses != 1:
        _set_status(report, STATUS_DB_READ_FAILED, "analysis.count")
        return None
    if counts.judge_outputs != 1:
        _set_status(report, STATUS_DB_READ_FAILED, "judge_output.count")
        return None
    if counts.candidates != 1:
        _set_status(report, STATUS_DB_READ_FAILED, "candidate.count")
        return None
    if counts.maintenance_sent_success_markers:
        _set_status(report, STATUS_EXISTING_MARKER, "maintenance_sent_success_marker.existing")
        return None
...
async def _load_counts(session: base.AsyncSessionLike, target: DeliveryResultTarget) -> TargetCounts:
    ...
    return TargetCounts(
        ...
        maintenance_sent_success_markers=await base._count_query(
            session, COUNT_MAINTENANCE_SENT_SUCCESS_MARKER_FOR_TARGET_QUERY, params
        ),
...
def _merge_existing_count_report(report: dict[str, Any], counts: TargetCounts) -> None:
    ...
    report["existing_maintenance_sent_success_marker_bucket"] = base._bucket_count(
        counts.maintenance_sent_success_markers
    )
...
def _merge_written_count_report(report: dict[str, Any], *, before: TargetCounts, after: TargetCounts) -> None:
    report["job_attempts_written_bucket"] = base._bucket_count(
        after.maintenance_sent_success_markers - before.maintenance_sent_success_markers
    )
...
async def _run_maintenance_marker_write(
    *,
    ...
    decision = classify_delivery_result_sent_success(delivery_status=target.delivery_status)
    if decision.action != "mark_terminal_success":
        raise ExpectedEffectsError("target delivery result is not sent")

    if before_counts.maintenance_sent_success_markers:
        raise ExpectedEffectsError("sent-success marker already exists")
    ...
    async with session.begin():
        ...
        await repository.insert_delivery_result_sent_success_job_attempt(target.notification_delivery_record_id)
        ...
        if not _approved_execution_succeeded(report):
            raise ExpectedEffectsError("maintenance sent-success effects did not match contract")


def _approved_execution_succeeded(report: Mapping[str, Any]) -> bool:
    return bool(
        report["maintenance_started"] is True
        and report["maintenance_classification_bucket"] == DELIVERY_RESULT_SENT_SUCCESS_CLASSIFICATION
        and report["job_attempts_written_bucket"] == "one"
        and report["target_delivery_status_bucket"] == "sent"
        ...
        and report["telegram_bot_token_read_bucket"] == "zero"
