...
SMOKE_MARKER_PREFIX = "ops-smoke:delivery-gate:"
MUTATION_SAFETY = (
    "controlled smoke write only: inserts marker-scoped synthetic source, artifact, "
    "candidate, bundle, judge, analysis, notification_plan, and one sent "
    "notification_delivery_records fixture row; runs the existing DeliveryGateRunner "
    "once in restricted mode; does not start runtime workers; does not publish Redis "
    "messages; does not mutate feature flags, environment files, notification renders, "
    "delivery side effects, dead letters, replay requests, state transitions, or event_outbox"
)
LOCKED_RESTRICTED_METRIC_ORDER = [
    "success_rate_1h",
...
CURRENT_COMPATIBILITY_METRICS = ["plan_to_transport_p95_sec"]
ALLOWED_RECOMMENDED_FLAG_PATCH_KEYS = [
    "ENABLE_NOTIFICATION_SEND",
    "MAINTENANCE_ENABLE_NOTIFICATION_RETRY_PROMOTION",
    "NOTIFIER_TELEGRAM_DRY_RUN",
]
FORBIDDEN_RECOMMENDED_FLAG_PATCH_KEYS = [
    "RESTRICTED_SCOPE_REVIEW_REQUIRED",
...
class SmokeReport:
    ...
    metric_order_core_observed: list[str]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run an opt-in post-Stage44 delivery-gate DB acceptance smoke. "
    ...
    parser.add_argument(
        "--database-url",
        default=None,
        help=f"Smoke database URL. Defaults to ${DATABASE_URL_ENV}. Redis is not required.",
    )
...
def _build_seed_shape(smoke_id: str | None = None) -> SmokeSeedShape:
    ...
    repo_name = f"delivery-gate-smoke-{short}"
    ...
    return SmokeSeedShape(
        ...
        material_change_hash=hashlib.sha256(f"delivery-gate:{smoke_id}".encode("utf-8")).hexdigest(),
...
def _new_report(seed_shape: SmokeSeedShape | None = None) -> SmokeReport:
    ...
    return SmokeReport(
        ...
        warnings=[
            ...
            "Redis is not required because the delivery gate runner is a DB-backed one-shot report path.",
            "It does not call OpenAI, GitHub, X, Telegram Bot API, or any external network.",
            "It does not start collector, outbox-relay, router-normalizer, enrichers, assembler, analysis, judge, policy, notifier, or maintenance workers.",
        ...
        gate_report_summary={},
        metric_order_expected=list(LOCKED_RESTRICTED_METRIC_ORDER),
...
def _delivery_gate_config(database_url: str) -> MaintenanceConfig:
    return MaintenanceConfig(
        ...
        maintenance_consumer_group="delivery-gate-runtime-smoke",
        maintenance_consumer_name="delivery-gate-runtime-smoke-1",
        replay_queue_name="q.replay",
        replay_consumer_group="delivery-gate-runtime-smoke-replay",
        replay_consumer_name="delivery-gate-runtime-smoke-replay-1",
        batch_size=1,
        block_ms=1,
...
async def _insert_seed_rows(session: AsyncSession, *, seed_shape: SmokeSeedShape, seed_ids: SmokeSeedIds) -> None:
    ...
    text = f"Delivery gate runtime smoke for {seed_shape.canonical_url}"
    raw_message_json = {
    ...
    await session.execute(
        sa.text(
            ...
            INSERT INTO candidate_group_proposals (
                ...
                'proposed', 'delivery_gate_runtime_smoke_v1', :dedupe_subject_key,
                :created_at, :updated_at
            )
            """
        ),
    ...
    await session.execute(
        sa.text(
            ...
            INSERT INTO candidate_evidence_bundles (
                ...
                CAST(:artifact_id AS uuid), CAST(:artifact_id AS uuid),
                'delivery_gate_runtime_smoke_v1', :bundle_input_hash,
    ...
    await session.execute(
        sa.text(
            ...
            INSERT INTO judge_runs (
                ...
                'delivery_gate_runtime_smoke', 'gpt-5.4-mini', 'low',
                'delivery_gate_runtime_smoke_v1', 'judge_output_v1', 'verdict_policy_v1',
        ...
        ),
        {
            ...
            "bundle_id": str(seed_ids.bundle_id),
            "prompt_cache_key": f"delivery-gate:{seed_shape.smoke_id}",
    ...
    await session.execute(
        sa.text(
            ...
            INSERT INTO analyses (
                ...
                'delivery_gate_runtime_smoke_v1', 'delivery_policy_v1', 'later'::verdict_enum,
                'send_now'::delivery_decision_enum, CAST(:scores_json AS jsonb),
                CAST(:reason_codes_json AS jsonb), 'synthetic smoke fixture',
                'restricted pass acceptance smoke only', 'deterministic fixture',
...
def _restricted_core_metric_order(metric_order: list[str]) -> list[str]:
    return [metric for metric in metric_order if metric not in CURRENT_COMPATIBILITY_METRICS]
...
def _check_preconditions(report: SmokeReport, *, database_url: str) -> bool:
    blocking_counts = {
        name: count
        for name, count in report.db_precondition_counts.items()
        if name
        in {
            "non_marker_recent_delivery_records_24h",
            "non_marker_due_retry_plans",
            "open_delivery_dlq_count",
            "unexpected_send_disabled_count",
        }
        and count != 0
    ...
    if blocking_counts:
        _mark_fail(
            report,
            "db.precondition_clean_restricted_gate_window",
            (
                "restricted_pass_minimal requires an otherwise clean local gate window; "
                f"interfering counts: {blocking_counts}"
            ),
            database_url=database_url,
        )
        return False
    _mark_pass(report, "db.precondition_clean_restricted_gate_window")
    return True


def _check_gate_report(report: SmokeReport, *, database_url: str) -> None:
    if report.gate_status == "pass":
        _mark_pass(report, "gate.gate_status_pass")
    else:
        _mark_fail(report, "gate.gate_status_pass", f"gate_status was {report.gate_status!r}", database_url=database_url)

    if report.blocking_reason_codes == []:
        _mark_pass(report, "gate.blocking_reason_codes_empty")
    else:
        _mark_fail(
            report,
            "gate.blocking_reason_codes_empty",
            f"blocking_reason_codes was {report.blocking_reason_codes!r}",
            database_url=database_url,
        )

    if report.warning_reason_codes == []:
        _mark_pass(report, "gate.warning_reason_codes_empty")
    else:
        _mark_fail(
            report,
            "gate.warning_reason_codes_empty",
            f"warning_reason_codes was {report.warning_reason_codes!r}",
            database_url=database_url,
        )

    if report.metric_order_core_observed == LOCKED_RESTRICTED_METRIC_ORDER:
        _mark_pass(report, "gate.locked_restricted_metric_order_observed")
    else:
        _mark_fail(
            report,
            "gate.locked_restricted_metric_order_observed",
            (
                "locked restricted metric order mismatch: "
                f"expected {LOCKED_RESTRICTED_METRIC_ORDER!r}, got {report.metric_order_core_observed!r}; "
                f"actual observed order was {report.metric_order_observed!r}"
            ),
            database_url=database_url,
        )

    observed_keys = list(report.recommended_flag_patch)
    if observed_keys == ALLOWED_RECOMMENDED_FLAG_PATCH_KEYS:
        _mark_pass(report, "gate.recommended_flag_patch_allowed_keys_exact")
    else:
        _mark_fail(
            report,
            "gate.recommended_flag_patch_allowed_keys_exact",
            f"recommended_flag_patch keys were {observed_keys!r}",
            database_url=database_url,
        )

    forbidden_present = [key for key in FORBIDDEN_RECOMMENDED_FLAG_PATCH_KEYS if key in report.recommended_flag_patch]
    if not forbidden_present:
        _mark_pass(report, "gate.recommended_flag_patch_forbidden_keys_absent")
    else:
        _mark_fail(
            report,
            "gate.recommended_flag_patch_forbidden_keys_absent",
            f"forbidden recommended flag patch keys were present: {forbidden_present!r}",
            database_url=database_url,
        )
...
async def _run_smoke(database_url: str, report: SmokeReport, seed_shape: SmokeSeedShape) -> SmokeReport:
    ...
    try:
        ...
        async with session_factory() as session:
            ...
            gate_report = await runner.run(mode="restricted")
            report.gate_report_summary = _gate_report_summary(gate_report)
            report.metric_order_observed = [metric.metric_name for metric in gate_report.metrics]
            report.metric_order_core_observed = _restricted_core_metric_order(report.metric_order_observed)
            report.gate_status = gate_report.gate_status
            report.blocking_reason_codes = list(gate_report.blocking_reason_codes)
            report.warning_reason_codes = list(gate_report.warning_reason_codes)
            report.recommended_flag_patch = dict(gate_report.recommended_flag_patch)
            _mark_pass(report, "gate.delivery_gate_report_exists")
            _check_gate_report(report, database_url=database_url)
