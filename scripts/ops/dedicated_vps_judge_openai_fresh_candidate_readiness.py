...
SCHEMA_VERSION = "1.0"
SCRIPT_NAME = "dedicated_vps_judge_openai_fresh_candidate_readiness"
REPORT_TYPE = "judge_openai_fresh_candidate_readiness_v1"
...
STATUS_PREFLIGHT_PASSED = "judge_openai_fresh_candidate_readiness_preflight_passed"
STATUS_APPROVED_PREPARED = "judge_openai_fresh_candidate_readiness_approved_prepared"
STATUS_NOT_READY = "blocked_judge_openai_fresh_candidate_readiness_not_ready"
STATUS_MISSING_APPROVAL = (
    "blocked_judge_openai_fresh_candidate_readiness_missing_approval"
)
STATUS_NO_CANDIDATE = "blocked_judge_openai_fresh_candidate_readiness_no_candidate"
STATUS_AMBIGUOUS_CANDIDATE = (
    "blocked_judge_openai_fresh_candidate_readiness_ambiguous_candidate"
)
STATUS_EXISTING_ACTIVE_JUDGE_RUN = (
    "blocked_judge_openai_fresh_candidate_readiness_existing_active_judge_run"
)
STATUS_EXISTING_Q_CANDIDATE = (
    "blocked_judge_openai_fresh_candidate_readiness_existing_q_analysis_judge_candidate"
)
STATUS_EXISTING_OUTPUT = (
    "blocked_judge_openai_fresh_candidate_readiness_existing_output"
)
STATUS_EXISTING_READY_OUTBOX = (
    "blocked_judge_openai_fresh_candidate_readiness_existing_ready_outbox"
)
STATUS_FORBIDDEN_SIDE_EFFECT = (
    "blocked_judge_openai_fresh_candidate_readiness_forbidden_side_effect"
)
STATUS_WRITE_FAILED = "blocked_judge_openai_fresh_candidate_readiness_write_failed"
STATUS_REDIS_PUBLISH_FAILED = (
    "blocked_judge_openai_fresh_candidate_readiness_redis_publish_failed"
)
STATUS_RAW_VALUE_EMISSION = (
    "blocked_judge_openai_fresh_candidate_readiness_raw_value_emission"
)
...
@dataclass(frozen=True, slots=True)
class FreshCandidateApprovals:
    db_write: bool = False
    redis_publish: bool = False

    @property
    def all_granted(self) -> bool:
        return self.db_write and self.redis_publish

    @property
    def any_granted(self) -> bool:
        return self.db_write or self.redis_publish
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Prepare one fresh pending judge-openai candidate for the single-live-call "
            "smoke. Default mode is read-only; approved mode needs both DB-write and "
            "Redis-publish approvals."
        )
    )
...
def _base_report() -> dict[str, Any]:
    return {
        ...
        "eligible_candidate_found_bucket": "zero",
        "analysis_requested_event_found_bucket": "zero",
        "current_ready_bundle_found_bucket": "zero",
        "existing_active_judge_run_for_bundle_bucket": "zero",
        "existing_judge_output_for_selected_run_bucket": "zero",
        "existing_ready_outbox_for_selected_run_bucket": "zero",
        "existing_q_analysis_judge_candidate_bucket": "zero",
        "judge_run_created_bucket": "zero",
        "judge_call_requested_outbox_created_bucket": "zero",
...
def _extract_runtime_config(
    *,
    ...
    prompt_versions = {
        "github_primary": str(
            values.get("JUDGE_PROMPT_VERSION_GITHUB", PROMPT_VERSION_BY_PROFILE["github_primary"])
        ).strip()
        or PROMPT_VERSION_BY_PROFILE["github_primary"],
        "x_primary": str(
            values.get("JUDGE_PROMPT_VERSION_X", PROMPT_VERSION_BY_PROFILE["x_primary"])
        ).strip()
        or PROMPT_VERSION_BY_PROFILE["x_primary"],
        "text_idea_primary": str(
            values.get(
                "JUDGE_PROMPT_VERSION_TEXT_IDEA",
                PROMPT_VERSION_BY_PROFILE["text_idea_primary"],
            )
        ).strip()
        or PROMPT_VERSION_BY_PROFILE["text_idea_primary"],
    }
    return RuntimeConfig(
        database_url=database_url,
        redis_url=redis_url,
        xadd_maxlen=xadd_maxlen,
        default_model=str(values.get("JUDGE_DEFAULT_MODEL", DEFAULT_MODEL)).strip()
        or DEFAULT_MODEL,
        escalation_model=str(values.get("JUDGE_ESCALATION_MODEL", ESCALATION_MODEL)).strip()
        or ESCALATION_MODEL,
        default_reasoning_effort=str(
            values.get("JUDGE_REASONING_EFFORT_DEFAULT", DEFAULT_REASONING_EFFORT)
        ).strip()
        ...
        or ESCALATION_REASONING_EFFORT,
        enable_model_escalation=str(values.get("ENABLE_MODEL_ESCALATION", "false"))
        .strip()
        .lower()
        not in {"0", "false", "no", ""},
        prompt_versions=prompt_versions,
        schema_version=str(values.get("JUDGE_SCHEMA_VERSION", EXPECTED_SCHEMA_VERSION)).strip()
        or EXPECTED_SCHEMA_VERSION,
        policy_version=str(values.get("VERDICT_POLICY_VERSION", EXPECTED_POLICY_VERSION)).strip()
        or EXPECTED_POLICY_VERSION,
    )
...
def _decision_for_candidate(
    *,
    ...
    if profile not in ALLOWED_JUDGE_PROFILES:
        return None
    use_escalation = (
        config.enable_model_escalation
    ...
    prompt_version = config.prompt_versions[profile]
    schema_version = config.schema_version
    policy_version = config.policy_version
    return JudgeDecision(
        judge_profile=profile,
        model=config.escalation_model if use_escalation else config.default_model,
        reasoning_effort=(
...
def _approval_block_status(report: dict[str, Any], approvals: FreshCandidateApprovals) -> None:
...
def _raw_values_from_candidate(candidate: SelectedCandidate) -> set[str]:
    ...
    if candidate.existing_judge_run_id is not None:
        values.add(str(candidate.existing_judge_run_id))
...
async def _load_bundle(
    repository: "FreshCandidateRepository",
    bundle_id: UUID,
...
async def _evaluate_candidate(
    *,
    ...
    if not bundle.primary_summary:
        return None, None, None
    if profile is None:
        profile = _derive_profile_from_artifact_type(bundle.artifact_type)
    if profile not in ALLOWED_JUDGE_PROFILES:
        return None, None, None
    ...
    existing_runs = await repository.load_judge_runs_for_decision(
        bundle_id=bundle.bundle_id,
        decision=decision,
    )
    report["existing_active_judge_run_for_bundle_bucket"] = _bucket_count(
        len([run for run in existing_runs if run.status in {"pending", "running", "succeeded"}])
    )
    if len(existing_runs) > 1:
        return None, STATUS_AMBIGUOUS_CANDIDATE, "judge_run.multiple_existing"
    existing_judge_run_id: UUID | None = None
    if existing_runs:
        existing = existing_runs[0]
        if existing.status != "pending":
            return None, STATUS_EXISTING_ACTIVE_JUDGE_RUN, "judge_run.status"
        if not _judge_run_matches_decision(existing, bundle.bundle_id, decision):
            return None, STATUS_EXISTING_ACTIVE_JUDGE_RUN, "judge_run.contract"
        existing_judge_run_id = existing.judge_run_id
        output_count = await repository.count_judge_outputs_for_run(existing.judge_run_id)
        report["existing_judge_output_for_selected_run_bucket"] = _bucket_count(output_count)
        if output_count:
            return None, STATUS_EXISTING_OUTPUT, "judge_output.exists"
        ready_count = await repository.count_judge_output_ready_outbox_for_run(
            existing.judge_run_id
        )
        report["existing_ready_outbox_for_selected_run_bucket"] = _bucket_count(ready_count)
        if ready_count:
            return None, STATUS_EXISTING_READY_OUTBOX, "judge_output_ready_outbox.exists"
        downstream_analysis_count = await repository.count_analyses_for_run(existing.judge_run_id)
        downstream_notification_count = await repository.count_notification_rows_for_run(
            existing.judge_run_id
        )
        if downstream_analysis_count:
            return None, STATUS_FORBIDDEN_SIDE_EFFECT, "analysis.exists"
        if downstream_notification_count:
            return None, STATUS_FORBIDDEN_SIDE_EFFECT, "notification.exists"
...
async def _select_candidate(
    *,
    ...
    analysis_rows = await repository.fetch_pending_analysis_requested_events(limit=scan_limit)
    raw_values.update(_raw_values_from_event_rows(analysis_rows))
    event_candidates: list[SelectedCandidate] = []
    for row in analysis_rows:
        payload = row.payload_json
        bundle_id = _payload_uuid(payload, "bundle_id")
        candidate_group_id = _payload_uuid(payload, "candidate_group_id")
        if bundle_id is None or candidate_group_id is None:
            continue
        bundle = await _load_bundle(repository, bundle_id)
        if bundle is None or bundle.candidate_group_id != candidate_group_id:
            continue
        selected, status, check = await _evaluate_candidate(
            ...
            config=config,
            bundle=bundle,
            profile=_payload_str(payload, "judge_profile"),
            escalation_allowed=bool(payload.get("escalation_allowed", False)),
            source="analysis_requested_event",
            analysis_event_id=row.event_id,
        )
        if status is not None:
            return None, raw_values, status, check
        if selected is not None:
            event_candidates.append(selected)

    report["analysis_requested_event_found_bucket"] = _bucket_count(len(event_candidates))
    if len(event_candidates) > 1:
        return None, raw_values, STATUS_AMBIGUOUS_CANDIDATE, "analysis_requested.multiple"
    if len(event_candidates) == 1:
        report["current_ready_bundle_found_bucket"] = "one"
        report["eligible_candidate_found_bucket"] = "one"
        raw_values.update(_raw_values_from_candidate(event_candidates[0]))
        return event_candidates[0], raw_values, None, None
    ...
    report["current_ready_bundle_found_bucket"] = _bucket_count(len(bundle_candidates))
    if len(bundle_candidates) > 1:
        return None, raw_values, STATUS_AMBIGUOUS_CANDIDATE, "current_ready_bundle.multiple"
    if len(bundle_candidates) == 1:
        report["eligible_candidate_found_bucket"] = "one"
        raw_values.update(_raw_values_from_candidate(bundle_candidates[0]))
...
async def _prepare_and_publish(
    *,
    ...
    try:
        judge_run_id = candidate.existing_judge_run_id
        if judge_run_id is None:
            ...
            if judge_run_id is None:
                ...
                judge_run_id = existing.judge_run_id
            else:
                report["judge_run_created_bucket"] = "one"

        output_count = await repository.count_judge_outputs_for_run(judge_run_id)
        report["existing_judge_output_for_selected_run_bucket"] = _bucket_count(output_count)
        if output_count:
            await session.rollback()
            _set_status(report, STATUS_EXISTING_OUTPUT, "judge_output.exists")
            return ScriptResult(exit_code=1, report=report)
        ready_count = await repository.count_judge_output_ready_outbox_for_run(judge_run_id)
        report["existing_ready_outbox_for_selected_run_bucket"] = _bucket_count(ready_count)
        if ready_count:
            await session.rollback()
            _set_status(report, STATUS_EXISTING_READY_OUTBOX, "judge_output_ready_outbox.exists")
            return ScriptResult(exit_code=1, report=report)
        ...
        if len(outbox_rows) > 1:
            await session.rollback()
            _set_status(report, STATUS_AMBIGUOUS_CANDIDATE, "judge_call_requested.multiple")
            return ScriptResult(exit_code=1, report=report)
        if outbox_rows:
            ...
            if outbox_row is None:
                outbox_row = await repository.load_judge_call_outbox_by_dedupe_key(
                    f"judge-call:{judge_run_id}"
                )
            if outbox_row is None or outbox_row.status != "pending":
                await session.rollback()
                _set_status(report, STATUS_WRITE_FAILED, "judge_call_requested.insert")
                return ScriptResult(exit_code=1, report=report)
            report["judge_call_requested_outbox_created_bucket"] = "one"
...
class FreshCandidateRepository:
...
async def generate_report_async(
    *,
    ...
    approvals: FreshCandidateApprovals | None = None,
    ...
    approvals = approvals or FreshCandidateApprovals()
    report = _base_report()
    ...
    try:
        ...
        repository = FreshCandidateRepository(session)
...
def _approvals_from_args(args: argparse.Namespace) -> FreshCandidateApprovals:
    return FreshCandidateApprovals(
