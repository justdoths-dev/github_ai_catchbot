...
SMOKE_MARKER_PREFIX = "ops-smoke:batch-recovery-replay:"
EXPECTED_RECOVERY_MODE = "replay-selected"
EXPECTED_REPLAY_TYPE = "delivery"
EXPECTED_ROOT_OBJECT_TYPE = "notification_plan"
EXPECTED_REPLAY_STATUS = "requested"
EXPECTED_REQUESTED_BY = "ops-smoke"
EXPECTED_DELIVERY_STATUS = "failed_terminal"
...
EXPECTED_DUPLICATE_SKIP_REASONS = {
    "open_replay_request_exists",
    "open_replay_request_exists_at_insert",
}
MUTATION_SAFETY = (
    "controlled smoke write only: inserts marker-scoped synthetic source, artifact, "
    "candidate, bundle, judge, analysis, one failed_terminal notification_plan, "
    "and one failed_terminal notification_delivery_records fixture row; runs the "
    "existing DeliveryBatchRecoveryTool.replay_selected path twice; verifies one "
    "requested replay_requests row and the open replay request skip on duplicate "
    "rerun; does not require Redis, start runtime workers, publish Redis messages, "
    "call external network, mutate feature flags or env files, emit event_outbox "
    "notification retry-intents, create notification renders, create extra delivery "
    "records, create dead letters, or create state transitions"
)
JSON_ROW_COLUMNS = {
    "artifact_key_json",
...
NOTIFICATION_DELIVERY_RECORD_SNAPSHOT_FIELDS = (
...
class SmokeReport:
    ...
    replay_request_summary: dict[str, Any]
    replay_request_observed: dict[str, Any] | None
    notification_plan_snapshot_before: dict[str, Any] | None
    notification_plan_snapshot_after: dict[str, Any] | None
...
def _build_seed_shape(smoke_id: str | None = None) -> SmokeSeedShape:
    ...
    repo_name = f"batch-recovery-replay-smoke-{short}"
    return SmokeSeedShape(
        ...
        material_change_hash=hashlib.sha256(f"batch-recovery-replay:{smoke_id}".encode("utf-8")).hexdigest(),
    )
...
def _new_report(seed_shape: SmokeSeedShape | None = None) -> SmokeReport:
    ...
    return SmokeReport(
        ...
        warnings=[
            ...
            "This verifies replay-selected only; failed_retryable rows belong to retry-selected-due.",
        ],
        ...
        mutation_safety_fields={
            ...
            "event_outbox_retry_intents_created": False,
            "notification_plans_mutated_after_seed": False,
            "notification_delivery_records_mutated_after_seed": False,
        ...
        replay_request_observed=None,
        notification_plan_snapshot_before=None,
        notification_plan_snapshot_after=None,
    )
...
def _build_judge_output_payload(*, seed_ids: SmokeSeedIds, seed_shape: SmokeSeedShape) -> dict[str, Any]:
    return {
        ...
        "summary_ko": "Synthetic batch recovery replay smoke judge output.",
...
def _config(database_url: str) -> MaintenanceConfig:
    return MaintenanceConfig(
        ...
        maintenance_consumer_group="batch-recovery-replay-runtime-smoke",
        maintenance_consumer_name="batch-recovery-replay-runtime-smoke-1",
        replay_queue_name="q.replay",
        replay_consumer_group="batch-recovery-replay-runtime-smoke-replay",
        replay_consumer_name="batch-recovery-replay-runtime-smoke-replay-1",
        batch_size=1,
        block_ms=1,
...
async def _insert_seed_rows(session: AsyncSession, *, seed_shape: SmokeSeedShape, seed_ids: SmokeSeedIds) -> None:
    ...
    text = f"Batch recovery replay runtime smoke for {seed_shape.canonical_url}"
    raw_message_json = {
        "smoke_marker": seed_shape.marker,
        "source": REPORT_TYPE,
    ...
    await session.execute(
        ...
        ),
        {
            ...
            "artifact_id": str(seed_ids.artifact_id),
            "fetched_at": now,
            "content_anchor": hashlib.sha1(
                f"batch-recovery-replay-smoke:{seed_shape.smoke_id}".encode("utf-8")
            ).hexdigest(),
            "normalized_projection": json.dumps(
                {"summary": "Batch recovery replay runtime smoke snapshot", "smoke_marker": seed_shape.marker},
                sort_keys=True,
            ),
    ...
    await session.execute(
        ...
        ),
        {
            ...
            "source_message_id": str(seed_ids.source_message_id),
            "artifact_id": str(seed_ids.artifact_id),
            "normalizer_version": "batch-recovery-replay-runtime-smoke",
    ...
    bundle_input_hash = hashlib.sha256(f"batch-recovery-replay-bundle:{seed_shape.smoke_id}".encode("utf-8")).hexdigest()
    await session.execute(
        sa.text(
            """
            INSERT INTO candidate_evidence_bundles (
                ...
                'batch-recovery-replay-runtime-smoke-bundle-v1', :bundle_input_hash, 0,
            ...
            "primary_summary": json.dumps(
                {
                    ...
                    "canonical_id": seed_shape.canonical_id,
                    "canonical_url": seed_shape.canonical_url,
                    "summary": "Deterministic batch recovery replay runtime smoke evidence bundle.",
                    "smoke_marker": seed_shape.marker,
                },
                sort_keys=True,
            ),
    ...
    await session.execute(
        sa.text(
            ...
            INSERT INTO notification_plans (
                ...
                NULL, NULL, 'failed_terminal'::notification_status_enum, :created_at
            )
            """
        ),
        {
    ...
    await session.execute(
        sa.text(
            ...
            INSERT INTO notification_delivery_records (
                ...
                'failed_terminal'::notification_status_enum, 1,
        ...
        ),
        {
            ...
            "transport_error_code": "telegram_terminal_4xx",
            "transport_error_class": "terminal_transport",
            "telegram_response_json": json.dumps(
                {"smoke_marker": seed_shape.marker, "terminal": True, "external_network_calls_allowed": False},
                sort_keys=True,
            ),
            "created_at": now,
        },
...
async def _load_counts(session: AsyncSession, *, seed_ids: SmokeSeedIds, marker: str) -> dict[str, int]:
    ...
    return {
        ...
        "marker_notification_plan_created_intents": await _select_scalar(
            session,
            """
            SELECT COUNT(*)
            FROM event_outbox
...
async def _load_replay_request_rows(session: AsyncSession, *, notification_plan_id: UUID) -> list[dict[str, Any]]:
    result = await session.execute(
        sa.text(
            """
            SELECT replay_request_id, replay_type, root_object_type, root_object_id, requested_by, requested_at, status
...
async def _load_notification_delivery_record_snapshot(
    session: AsyncSession, *, notification_delivery_record_id: UUID
) -> dict[str, Any] | None:
    result = await session.execute(
        sa.text(
            """
...
async def _run_replay_selected(
    ...
    *,
    ...
    async with session_factory.begin() as session:
        ...
        result = await tool.replay_selected(plan_ids=[notification_plan_id], requested_by=EXPECTED_REQUESTED_BY)
...
def _verify_first_result(report: SmokeReport, *, result: dict[str, Any], database_url: str) -> None:
    ...
    if not mismatches:
        ...
        _mark_fail(
            ...
            f"first replay_selected result mismatches: {mismatches}",
            database_url=database_url,
        )


def _verify_second_result(report: SmokeReport, *, result: dict[str, Any], database_url: str) -> None:
    reason_codes = result.get("skipped_reason_codes") or {}
    has_expected_skip = any(reason_codes.get(reason) == 1 for reason in EXPECTED_DUPLICATE_SKIP_REASONS)
    expectations = {
        "recovery_mode": EXPECTED_RECOVERY_MODE,
        "selected_count": 1,
        "emitted_count": 0,
    ...
    if not mismatches and has_expected_skip:
        _mark_pass(report, "batch_recovery.second_result_idempotent_open_replay_skip")
    else:
        mismatches["skipped_reason_codes"] = {
            "expected_one_of": sorted(EXPECTED_DUPLICATE_SKIP_REASONS),
            "actual": reason_codes,
        }
        _mark_fail(
            report,
            "batch_recovery.second_result_idempotent_open_replay_skip",
            f"second replay_selected result mismatches: {mismatches}",
            database_url=database_url,
        )
...
def _verify_replay_request(
    ...
    *,
    ...
    if len(rows) == 1:
        _mark_pass(report, "db.exactly_one_replay_request_row")
    else:
        _mark_fail(
            report,
            "db.exactly_one_replay_request_row",
            f"expected one replay_requests row, got {len(rows)}",
            database_url=database_url,
        )
        return

    row = rows[0]
    report.replay_request_observed = dict(row)
    expected_row_values = {
        "replay_type": EXPECTED_REPLAY_TYPE,
        "root_object_type": EXPECTED_ROOT_OBJECT_TYPE,
        "root_object_id": str(seed_ids.notification_plan_id),
        "requested_by": EXPECTED_REQUESTED_BY,
        "status": EXPECTED_REPLAY_STATUS,
    }
    row_mismatches = {
    ...
    if not row_mismatches:
        _mark_pass(report, "db.replay_request_row_contract")
    else:
        _mark_fail(
            report,
            "db.replay_request_row_contract",
            f"replay_requests row mismatches: {row_mismatches}",
            database_url=database_url,
        )
...
def _verify_postcondition_counts(report: SmokeReport, *, database_url: str) -> None:
    expectations = {
        ...
        "marker_notification_plan_created_intents": 0,
        "marker_replay_requests": 1,
...
def _verify_notification_plan_row_unchanged_after_replay(
    report: SmokeReport,
    *,
    ...
    diff = _snapshot_diff(before, after, fields=NOTIFICATION_PLAN_SNAPSHOT_FIELDS)
    if not diff:
        _mark_pass(report, "db.notification_plan_row_unchanged_after_replay")
...
def _verify_notification_delivery_record_row_unchanged_after_replay(
    report: SmokeReport,
    *,
    before: Mapping[str, Any] | None,
    after: Mapping[str, Any] | None,
    ...
    diff = _snapshot_diff(before, after, fields=NOTIFICATION_DELIVERY_RECORD_SNAPSHOT_FIELDS)
    if not diff:
        _mark_pass(report, "db.notification_delivery_record_row_unchanged_after_replay")
        report.mutation_safety_fields["notification_delivery_records_mutated_after_seed"] = False
        return

    report.mutation_safety_fields["notification_delivery_records_mutated_after_seed"] = True
    _mark_fail(
        report,
        "db.notification_delivery_record_row_unchanged_after_replay",
        f"notification_delivery_records row changed after fixture seed: {diff}",
        database_url=database_url,
    )
...
async def _run_smoke(database_url: str, report: SmokeReport, seed_shape: SmokeSeedShape) -> SmokeReport:
    ...
    try:
        async with session_factory.begin() as session:
            ...
            delivery_record_snapshot_after_seed = await _load_notification_delivery_record_snapshot(
                session,
                notification_delivery_record_id=seed_ids.notification_delivery_record_id,
            )
            report.notification_plan_snapshot_before = notification_plan_snapshot_after_seed
        _mark_pass(report, "db.marker_scoped_fixture_inserted")

        first_result = await _run_replay_selected(
            session_factory,
            database_url=database_url,
            notification_plan_id=seed_ids.notification_plan_id,
        )
        ...
        async with session_factory() as session:
            first_rows = await _load_replay_request_rows(session, notification_plan_id=seed_ids.notification_plan_id)
        _verify_replay_request(report, rows=first_rows, seed_ids=seed_ids, database_url=database_url)

        second_result = await _run_replay_selected(
            session_factory,
            database_url=database_url,
            notification_plan_id=seed_ids.notification_plan_id,
        )
        ...
        async with session_factory() as session:
            final_rows = await _load_replay_request_rows(session, notification_plan_id=seed_ids.notification_plan_id)
            notification_plan_snapshot_after_replay = await _load_notification_plan_snapshot(
                session,
                notification_plan_id=seed_ids.notification_plan_id,
            )
            delivery_record_snapshot_after_replay = await _load_notification_delivery_record_snapshot(
                session,
                notification_delivery_record_id=seed_ids.notification_delivery_record_id,
            )
            report.db_postcondition_counts = await _load_counts(session, seed_ids=seed_ids, marker=seed_shape.marker)

        report.notification_plan_snapshot_after = notification_plan_snapshot_after_replay
        _verify_notification_plan_row_unchanged_after_replay(
            report,
            before=notification_plan_snapshot_after_seed,
            after=notification_plan_snapshot_after_replay,
            database_url=database_url,
        )
        _verify_notification_delivery_record_row_unchanged_after_replay(
            report,
            before=delivery_record_snapshot_after_seed,
            after=delivery_record_snapshot_after_replay,
            database_url=database_url,
        )
        report.replay_request_summary = {
            ...
            "replay_request_id": str(first_rows[0].get("replay_request_id")) if first_rows else None,
            "duplicate_rerun_result": second_result,
        }
        if len(final_rows) == 1:
            _mark_pass(report, "db.duplicate_rerun_inserted_no_second_replay_request_row")
        else:
            _mark_fail(
                report,
                "db.duplicate_rerun_inserted_no_second_replay_request_row",
                f"expected one final replay_requests row, got {len(final_rows)}",
                database_url=database_url,
            )
...
async def _amain(argv: list[str] | None = None) -> int:
    ...
    if _is_production_like_url(database_url) or not _is_expected_smoke_database_url(database_url):
        _mark_fail(
            report,
            "safety.local_smoke_database_url_required",
            "database URL must be a local PostgreSQL URL with a dev/test/smoke database name",
            database_url=database_url,
        )
        print(_render_json(report))
        return 1
    try:
        await _run_smoke(database_url, report, seed_shape)
    except Exception as exc:  # pragma: no cover - exercised by operator smoke failures.
        _mark_fail(
            report,
