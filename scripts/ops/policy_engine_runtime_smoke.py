...
from src.services.policy_engine.verdict_policy import VerdictPolicy
from src.services.policy_engine.worker import PolicyEngineWorker


REPORT_TYPE = "policy_engine_runtime_smoke_v1"
...
MUTATION_SAFETY = (
    ...
    "candidate, ready evidence bundle/member, succeeded judge_run, valid judge_output, and one "
    "pending analysis.policy.apply.v1 outbox row; publishes that event through the existing "
    "outbox relay route to q.analysis.policy; runs one policy-engine worker pass; and leaves "
    "one analysis row, one policy success state transition, and one pending "
    "notification.plan.created.v1 outbox intent for the non-suppress result"
)
SMOKE_MARKER_PREFIX = "ops-smoke:policy-engine-runtime:"
EXPECTED_QUEUE_NAME = "q.analysis.policy"
EXPECTED_STAGE_NAME = "analysis_policy"
EXPECTED_REDIS_DB = 14
EXPECTED_EVENT_TYPE = "analysis.policy.apply.v1"
EXPECTED_DOWNSTREAM_EVENT_TYPE = "notification.plan.created.v1"
EXPECTED_AGGREGATE_TYPE = "judge_run"
EXPECTED_ARTIFACT_TYPE = "github_repo"
EXPECTED_OWNER = "octocat"
EXPECTED_REPO_PREFIX = "policy-engine-smoke"
...
EXPECTED_BUNDLE_PROFILE_VERSION = "policy-engine-runtime-smoke-bundle-v1"
EXPECTED_TOKEN_BUDGET_PROFILE = "small"
EXPECTED_FINISH_REASON = "completed"
EXPECTED_FINAL_VERDICT = "later"
EXPECTED_MODEL_PROPOSED_VERDICT = "skip"
EXPECTED_DELIVERY_DECISION = "send_now"
EXPECTED_URGENCY_PROFILE = "normal_silent"
EXPECTED_OPERATOR_CHAT_ID = 12345
...
class SmokeReport:
    ...
    notification_plan_created_outbox_ids: list[str]
...
class SessionBackedPolicyEngineService:
    def __init__(
        self,
        *,
        ...
        async with self._session_factory.begin() as session:
            ...
            service = PolicyEngineService(
                ...
                delivery_policy=DeliveryPolicy(
                    enable_later_delivery=self._config.enable_later_delivery,
                    enable_silent_later=self._config.enable_silent_later,
                ),
                notification_intent_builder=NotificationIntentBuilder(config=self._config),
                logger=self._logger,
            )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run an opt-in post-Stage44 policy-engine runtime smoke. "
...
def _build_seed_shape(smoke_id: str | None = None) -> SmokeSeedShape:
    ...
    content_anchor = hashlib.sha1(f"policy-engine-runtime-smoke:{smoke_id}".encode("utf-8")).hexdigest()
    bundle_input_hash = hashlib.sha256(f"policy-engine-bundle:{smoke_id}".encode("utf-8")).hexdigest()
...
def _new_report(seed_shape: SmokeSeedShape | None = None) -> SmokeReport:
    ...
    return SmokeReport(
        ...
        warnings=[
            ...
            "It does not start the notifier worker and does not create notification render or delivery rows.",
...
def _build_expected_redis_payload(*, event_id: UUID, judge_run_id: UUID, marker: str) -> dict[str, str]:
    return {
        ...
        "idempotency_key": f"{marker}:analysis-policy-apply",
        "pipeline_run_id": "",
        "not_before": "",
...
def _build_policy_apply_payload(*, seed_ids: SmokeSeedIds, seed_shape: SmokeSeedShape) -> dict[str, Any]:
    return {
...
def _build_valid_judge_output_payload(*, seed_ids: SmokeSeedIds, seed_shape: SmokeSeedShape) -> dict[str, Any]:
    return {
        ...
        "candidate_group_id": str(seed_ids.candidate_group_id),
        "headline": f"Runtime smoke policy output for {seed_shape.owner}/{seed_shape.repo_name}",
        "summary_one_line_ko": "Deterministic policy-engine smoke summary.",
        "skeptical_take_ko": "Synthetic evidence only; not a production verdict.",
        "why_it_might_matter_ko": "Exercises policy-engine verdict and notification intent boundary.",
...
async def _insert_seed_rows(session: AsyncSession, *, seed_shape: SmokeSeedShape, seed_ids: SmokeSeedIds) -> None:
    ...
    message_id = int(str(seed_ids.policy_apply_event_id.int)[-9:]) or 1
    text = f"Runtime smoke for {seed_shape.canonical_url}"
    raw_message_json = {
        "smoke_marker": seed_shape.marker,
        "source": "policy_engine_runtime_smoke",
    ...
    normalized_projection = {
        ...
        "description": "Deterministic policy-engine runtime smoke repository fixture",
        "language": "Python",
        "stars": 42,
    ...
    await session.execute(
        ...
        ),
        {
            ...
            "source_message_id": str(seed_ids.source_message_id),
            "artifact_id": str(seed_ids.artifact_id),
            "normalizer_version": "policy-engine-runtime-smoke",
    ...
    primary_summary = {
        ...
        "summary": "Deterministic GitHub repository summary for policy-engine runtime smoke.",
    ...
    policy_payload = _build_policy_apply_payload(seed_ids=seed_ids, seed_shape=seed_shape)
    await session.execute(
        sa.text(
            """
        ...
        {
            "event_id": str(seed_ids.policy_apply_event_id),
            "event_type": EXPECTED_EVENT_TYPE,
            "aggregate_type": EXPECTED_AGGREGATE_TYPE,
            "aggregate_id": str(seed_ids.judge_run_id),
            "dedupe_key": f"{seed_shape.marker}:analysis-policy-apply",
...
async def _load_bundle_snapshot(session: AsyncSession, *, bundle_id: UUID) -> dict[str, Any]:
    result = await session.execute(
        sa.text(
            """
    ...
    return {
        ...
        "bundle_profile_version": row["bundle_profile_version"],
        "bundle_input_hash": row["bundle_input_hash"],
        "reroot_count": row["reroot_count"],
        "primary_summary": _json_loads(row["primary_summary"]),
        "supporting_summaries_json": _json_loads(row["supporting_summaries_json"]),
        "discovered_links_summary_json": _json_loads(row["discovered_links_summary_json"]),
        "evidence_limitations": _json_loads(row["evidence_limitations"]),
        "ready_for_analysis": row["ready_for_analysis"],
        "token_budget_profile": row["token_budget_profile"],
        "created_at": row["created_at"],
    }


async def _load_candidate_current_analysis_id(session: AsyncSession, *, candidate_group_id: UUID) -> str | None:
...
async def _load_outputs(session: AsyncSession, *, seed_ids: SmokeSeedIds) -> dict[str, Any]:
    ...
    outputs["analyses"] = [_normalize_analysis_row(row) for row in analysis_result.mappings().all()]
    ...
    outputs["notification_plan_created_outbox"] = [dict(row) for row in notify_result.mappings().all()]
    outputs["policy_apply_status"] = await _select_scalar(
        session,
        """
        SELECT status
        FROM event_outbox
...
def _normalize_analysis_row(row: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "delivery_policy_version": row["delivery_policy_version"],
        "verdict": str(row["verdict"]),
        "delivery_decision": str(row["delivery_decision"]),
        "scores_json": _json_loads(row["scores_json"]),
        "reason_codes_json": _json_loads(row["reason_codes_json"]),
...
def _quiet_logger() -> logging.Logger:
    logger = logging.getLogger("policy-engine-runtime-smoke.worker")
    logger.handlers = [logging.NullHandler()]
    logger.propagate = False
    return logger


def _config(*, database_url: str, redis_url: str, consumer_group: str, consumer_name: str) -> PolicyEngineConfig:
    return PolicyEngineConfig(
        ...
        policy_version=EXPECTED_POLICY_VERSION,
        delivery_policy_version=EXPECTED_DELIVERY_POLICY_VERSION,
        operator_chat_id=EXPECTED_OPERATOR_CHAT_ID,
...
async def run_smoke(*, database_url: str, redis_url: str) -> SmokeReport:
    ...
    report.seeded_ids = {
        ...
        "judge_run_id": str(seed_ids.judge_run_id),
        "judge_output_id": str(seed_ids.judge_output_id),
        "policy_apply_event_id": str(seed_ids.policy_apply_event_id),
        "canonical_id": seed_shape.canonical_id,
    }
    ...
    consumer_group = f"policy-engine-smoke-{seed_shape.smoke_id[:12]}"
    consumer_name = f"{consumer_group}-1"
    before_judge_output: dict[str, Any] | None = None
    before_evidence_bundle: dict[str, Any] | None = None
    before_candidate_current_analysis_id: str | None = None
    try:
        ...
        async with AsyncSession(engine, expire_on_commit=False) as seed_session:
            async with seed_session.begin():
                ...
                before_evidence_bundle = await _load_bundle_snapshot(seed_session, bundle_id=seed_ids.bundle_id)
                before_candidate_current_analysis_id = await _load_candidate_current_analysis_id(
                    seed_session,
        ...
        async with AsyncSession(engine, expire_on_commit=False) as relay_session:
            async with relay_session.begin():
                stream_message_id, expected_fields, route = await _publish_seed_event_through_outbox_route(
                    ...
                    event_id=seed_ids.policy_apply_event_id,
                )
        ...
        if route.queue_name == EXPECTED_QUEUE_NAME and route.stage_name == EXPECTED_STAGE_NAME:
            _mark_pass(report, "outbox_relay.route_published_policy_apply_to_analysis_policy")
        else:
            _mark_fail(
                report,
                "outbox_relay.route_published_policy_apply_to_analysis_policy",
        ...
        if expected_fields == _build_expected_redis_payload(
            ...
            _mark_fail(
                ...
                "outbox relay stream fields did not match the expected policy-engine thin payload",
                database_url=database_url,
                redis_url=redis_url,
            )
            return report
        ...
        payload_failures = validate_redis_payload(
            redis_fields,
            expected_event_id=seed_ids.policy_apply_event_id,
        ...
        service = SessionBackedPolicyEngineService(
            config=config,
        ...
        if batch_result.processed == 1 and batch_result.acked == 1:
            _mark_pass(report, "policy_engine.worker_consumed_and_acked_one_message")
        else:
            _mark_fail(
                report,
                "policy_engine.worker_consumed_and_acked_one_message",
        ...
        async with AsyncSession(engine, expire_on_commit=False) as verify_session:
            ...
            after_evidence_bundle = await _load_bundle_snapshot(verify_session, bundle_id=seed_ids.bundle_id)
            after_candidate_current_analysis_id = await _load_candidate_current_analysis_id(
                verify_session,
                candidate_group_id=seed_ids.candidate_group_id,
            )

        if str(outputs.get("policy_apply_status")) == "published":
            _mark_pass(report, "db.analysis_policy_apply_outbox_published")
        else:
            _mark_fail(
                report,
                "db.analysis_policy_apply_outbox_published",
                f"expected seeded analysis.policy.apply.v1 to be published, got {outputs.get('policy_apply_status')!r}",
                database_url=database_url,
                redis_url=redis_url,
            )

        analyses = outputs.get("analyses") or []
        report.analysis_row = analyses[0] if analyses else {}
        if len(analyses) == 1:
            _mark_pass(report, "db.exactly_one_analysis_row_exists")
        else:
            _mark_fail(
                report,
                "db.exactly_one_analysis_row_exists",
                f"expected exactly one analyses row for seeded judge_output/policies, got {len(analyses)}",
                database_url=database_url,
                redis_url=redis_url,
            )
        ...
        if analyses:
            ...
            analysis_ok = all(analysis.get(key) == value for key, value in required_values.items())
            analysis_ok = analysis_ok and analysis.get("scores_json") == _build_valid_judge_output_payload(
                seed_ids=seed_ids,
                seed_shape=seed_shape,
            )["scores"]
            reason_codes = analysis.get("reason_codes_json") or []
            analysis_ok = analysis_ok and "policy_threshold_later" in reason_codes
            analysis_ok = analysis_ok and "policy_overrode_model_verdict" in reason_codes
            analysis_ok = analysis_ok and analysis.get("policy_reconciled_flag") is False
        if analysis_ok:
            _mark_pass(report, "db.analysis_row_contract")
        else:
            _mark_fail(
                report,
                "db.analysis_row_contract",
                f"analysis row did not match policy-engine contract: {analyses}",
                database_url=database_url,
                redis_url=redis_url,
            )

        if analyses and analyses[0].get("verdict") != analyses[0].get("model_proposed_verdict"):
            _mark_pass(report, "db.verdict_deterministically_recomputed_not_copied")
        else:
            _mark_fail(
                report,
                "db.verdict_deterministically_recomputed_not_copied",
                "expected final verdict to differ from model_proposed_verdict for seeded scores",
                database_url=database_url,
                redis_url=redis_url,
            )
        ...
        success_transitions = [
            ...
            if row.get("to_state") == "analysis_finalized"
            and row.get("reason_code") == f"policy_applied:{EXPECTED_FINAL_VERDICT}:{EXPECTED_DELIVERY_DECISION}"
        ]
        if len(success_transitions) == 1:
            _mark_pass(report, "db.policy_success_state_transition_exists")
        else:
            _mark_fail(
                report,
                "db.policy_success_state_transition_exists",
                f"expected one policy success transition, got {transitions}",
                database_url=database_url,
                redis_url=redis_url,
            )

        notify_outbox = outputs.get("notification_plan_created_outbox") or []
        report.notification_plan_created_outbox_ids = [str(row["event_id"]) for row in notify_outbox]
        pending_notify = [row for row in notify_outbox if row["status"] == "pending"]
        if len(pending_notify) == 1:
            _mark_pass(report, "db.notification_plan_created_outbox_pending_exists")
        else:
            _mark_fail(
                report,
                "db.notification_plan_created_outbox_pending_exists",
                f"expected one pending notification.plan.created.v1 outbox row, got {len(pending_notify)}",
                database_url=database_url,
                redis_url=redis_url,
            )

        notify_payload_ok = False
        if pending_notify and analyses:
            payload = _json_loads(pending_notify[0]["payload_json"]) or {}
            notify_payload_ok = {
                "analysis_id": payload.get("analysis_id"),
                "candidate_group_id": payload.get("candidate_group_id"),
                "delivery_decision": payload.get("delivery_decision"),
                "urgency_profile": payload.get("urgency_profile"),
                "target_chat_id": payload.get("target_chat_id"),
                "render_profile": payload.get("render_profile"),
                "dedupe_subject_key": payload.get("dedupe_subject_key"),
                ...
                "target_chat_id": EXPECTED_OPERATOR_CHAT_ID,
            ...
            notify_payload_ok = notify_payload_ok and bool(payload.get("notification_plan_id"))
            notify_payload_ok = notify_payload_ok and bool(payload.get("material_change_hash"))
            notify_payload_ok = notify_payload_ok and bool(payload.get("send_after"))
        if notify_payload_ok:
            _mark_pass(report, "db.notification_plan_created_payload_contract")
        else:
            _mark_fail(
                report,
                "db.notification_plan_created_payload_contract",
                f"notification.plan.created.v1 payload did not match policy-engine contract: {pending_notify}",
                database_url=database_url,
                redis_url=redis_url,
            )
        ...
        report.mutation_booleans = {
            ...
            "candidate_evidence_bundle_mutated": before_evidence_bundle != after_evidence_bundle,
            "candidate_current_analysis_id_mutated": before_candidate_current_analysis_id
            != after_candidate_current_analysis_id,
        }
        if not report.mutation_booleans["judge_output_mutated"]:
            ...
            _mark_fail(
                ...
                "judge_outputs row changed during policy-engine processing",
                database_url=database_url,
                redis_url=redis_url,
            )
        if not report.mutation_booleans["candidate_evidence_bundle_mutated"]:
            _mark_pass(report, "db.candidate_evidence_bundles_not_mutated")
        else:
            _mark_fail(
                report,
                "db.candidate_evidence_bundles_not_mutated",
                "candidate_evidence_bundles row changed during policy-engine processing",
                database_url=database_url,
                redis_url=redis_url,
            )
        if not report.mutation_booleans["candidate_current_analysis_id_mutated"]:
            _mark_pass(report, "db.candidate_current_analysis_id_not_mutated")
        else:
            _mark_fail(
                report,
                "db.candidate_current_analysis_id_not_mutated",
                "candidate_group_proposals.current_analysis_id changed during policy-engine processing",
                database_url=database_url,
                redis_url=redis_url,
            )
        ...
        report.db_postcondition_counts = {
            ...
            "policy_success_transitions": len(success_transitions),
            "notification_plan_created_pending": len(pending_notify),
            **side_effect_counts,
        }
        if all(count == 0 for count in side_effect_counts.values()):
            _mark_pass(report, "db.no_forbidden_notifier_side_effects")
        else:
            _mark_fail(
                report,
                "db.no_forbidden_notifier_side_effects",
                f"unexpected notifier-owned side-effect rows found: {side_effect_counts}",
                database_url=database_url,
                redis_url=redis_url,
            )
    finally:
