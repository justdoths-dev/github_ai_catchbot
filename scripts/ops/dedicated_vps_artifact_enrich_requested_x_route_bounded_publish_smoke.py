...
SCHEMA_VERSION = "1.0"
SCRIPT_NAME = "dedicated_vps_artifact_enrich_requested_x_route_bounded_publish_smoke"
REPORT_TYPE = "artifact_enrich_requested_x_route_bounded_publish_smoke_v1"
...
EXPECTED_QUEUE_NAME = "q.artifact.enrich.x"
EXPECTED_STAGE_NAME = "enrich_x"
ALLOWED_AGGREGATE_TYPES = {"candidate_group", "artifact"}
...
STATUS_READY = "artifact_enrich_requested_x_route_bounded_publish_smoke_ready"
STATUS_PUBLISHED = "artifact_enrich_requested_x_route_bounded_publish_smoke_published"
STATUS_ALREADY_PUBLISHED = (
    "artifact_enrich_requested_x_route_bounded_publish_smoke_already_published"
)
STATUS_NO_PENDING = (
    "blocked_artifact_enrich_requested_x_route_bounded_publish_smoke_no_pending_x_enrich_event"
)
STATUS_INVALID_ROUTE = (
    "blocked_artifact_enrich_requested_x_route_bounded_publish_smoke_invalid_route"
)
STATUS_MISSING_APPROVAL = (
    "blocked_artifact_enrich_requested_x_route_bounded_publish_smoke_missing_approval"
)
STATUS_FORBIDDEN_SIDE_EFFECT = "blocked_forbidden_side_effect_detected"
STATUS_NOT_READY = "blocked_artifact_enrich_requested_x_route_bounded_publish_smoke_not_ready"
STATUS_REDIS_PUBLISH_FAILED = (
    "blocked_artifact_enrich_requested_x_route_bounded_publish_smoke_redis_publish_failed"
)
STATUS_DB_WRITE_FAILED_AFTER_REDIS = (
    "blocked_artifact_enrich_requested_x_route_bounded_publish_smoke_db_write_failed_after_redis_publish"
)
...
SELECT_PENDING_ENRICH_EVENTS_QUERY = """
SELECT
    event_id,
    event_type,
    aggregate_type,
...
WHERE status = 'pending'::outbox_status_enum
  ...
  AND aggregate_type IN ('candidate_group', 'artifact')
ORDER BY created_at DESC, event_id DESC
LIMIT :limit
"""
COUNT_PUBLISHED_X_ENRICH_EVENTS_QUERY = """
SELECT COUNT(*)
FROM event_outbox
WHERE event_type = 'artifact.enrich.requested.v1'
  AND status::text = 'published'
  AND payload_json->>'provider_route' = 'x'
"""
VALIDATE_CANDIDATE_GROUP_AGGREGATE_QUERY = """
SELECT COUNT(*)
...
"""
VALIDATE_ARTIFACT_AGGREGATE_WITH_GROUP_QUERY = """
SELECT COUNT(*)
...
WHERE cgm.artifact_id = CAST(:artifact_id AS uuid)
  AND cgp.candidate_group_id = CAST(:candidate_group_id AS uuid)
"""
VALIDATE_ARTIFACT_AGGREGATE_QUERY = """
SELECT COUNT(*)
FROM candidate_group_members cgm
JOIN candidate_group_proposals cgp
  ON cgp.candidate_group_id = cgm.candidate_group_id
...
class PublishApprovals:
    artifact_enrich_x_route_publish_smoke: bool
    redis_publish: bool
    event_outbox_published_update: bool
    job_attempt_write: bool

    @property
    def all_granted(self) -> bool:
        return (
            self.artifact_enrich_x_route_publish_smoke
            and self.redis_publish
            and self.event_outbox_published_update
    ...
    def any_granted(self) -> bool:
        return (
            ...
            or self.event_outbox_published_update
            or self.job_attempt_write
        )

    def missing_checks(self) -> list[str]:
        checks: list[str] = []
        if not self.artifact_enrich_x_route_publish_smoke:
            checks.append("approval.artifact_enrich_x_route_publish_smoke")
        if not self.redis_publish:
            checks.append("approval.redis_publish")
        if not self.event_outbox_published_update:
            checks.append("approval.event_outbox_published_update")
...
class ArtifactEnrichOutboxRepository(OutboxRelayRepository):
    async def fetch_recent_pending_enrich_events(self, *, limit: int) -> list[OutboxEventRow]:
        result = await self._session.execute(
            _sql(SELECT_PENDING_ENRICH_EVENTS_QUERY),
            {"limit": limit},
        )
    ...
    async def count_published_x_enrich_events(self) -> int:
        value = await _scalar(
            await self._session.execute(_sql(COUNT_PUBLISHED_X_ENRICH_EVENTS_QUERY), {})
        )
        return _safe_count(value)

    async def candidate_group_aggregate_is_consistent(
        self,
        *,
        ...
        value = await _scalar(
            await self._session.execute(
                _sql(VALIDATE_CANDIDATE_GROUP_AGGREGATE_QUERY),
                {
    ...
    async def artifact_aggregate_is_consistent(
        self,
        *,
        artifact_id: str,
        ...
        if candidate_group_id:
            value = await _scalar(
                await self._session.execute(
                    _sql(VALIDATE_ARTIFACT_AGGREGATE_WITH_GROUP_QUERY),
                    {
                        "candidate_group_id": candidate_group_id,
                        "artifact_id": artifact_id,
                    },
            ...
            value = await _scalar(
                await self._session.execute(
                    _sql(VALIDATE_ARTIFACT_AGGREGATE_QUERY),
                    {"artifact_id": artifact_id},
                )
            )
        return _safe_count(value) > 0
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Bounded artifact.enrich.requested.v1 X-route outbox publish smoke. "
            "Default mode reads runtime configuration, validates one pending X-route "
            "enrich event, and inspects Redis without mutation."
        )
    )
    ...
    parser.add_argument(
        "--approved-artifact-enrich-x-route-publish-smoke",
        action="store_true",
    )
    parser.add_argument("--approved-redis-publish", action="store_true")
    parser.add_argument("--approved-event-outbox-published-update", action="store_true")
...
def _base_report() -> dict[str, Any]:
    return {
        ...
        "pending_x_enrich_event_selected_bucket": "zero",
        "provider_route_x_bucket": "zero",
        "route_queue_valid_bucket": "zero",
        "route_stage_valid_bucket": "zero",
...
async def _event_shape_is_consistent(
    *,
    repository: ArtifactEnrichOutboxRepository,
    row: OutboxEventRow,
) -> tuple[bool, str | None]:
    payload = row.payload_json
    artifact_id = _payload_str(payload, "artifact_id")
    candidate_group_id = _payload_str(payload, "candidate_group_id")
    if row.event_type != EXPECTED_EVENT_TYPE:
        return False, "event_outbox.event_type"
    if row.status != "pending":
        return False, "event_outbox.status"
    if row.aggregate_type not in ALLOWED_AGGREGATE_TYPES:
        return False, "event_outbox.aggregate_type"
    if not artifact_id:
        return False, "payload.artifact_id"

    if row.aggregate_type == "candidate_group":
        if not candidate_group_id:
            return False, "payload.candidate_group_id"
        if candidate_group_id != str(row.aggregate_id):
            return False, "aggregate.candidate_group_id"
        if not await repository.candidate_group_aggregate_is_consistent(
            candidate_group_id=candidate_group_id,
            artifact_id=artifact_id,
        ):
            return False, "aggregate.candidate_group_membership"
        return True, None

    if artifact_id != str(row.aggregate_id):
        return False, "aggregate.artifact_id"
    if not await repository.artifact_aggregate_is_consistent(
        artifact_id=artifact_id,
        candidate_group_id=candidate_group_id,
    ):
        return False, "aggregate.artifact_membership"
    return True, None
...
async def _select_target(
    *,
    ...
    rows = await repository.fetch_recent_pending_enrich_events(limit=pending_scan_limit)
    raw_values = _raw_values_from_rows(rows)
    provider_route_x_count = 0
    route_queue_valid_count = 0
    route_stage_valid_count = 0
    thin_payload_valid_count = 0
    invalid_checks: list[str] = []

    for row in rows:
        provider_route = _payload_str(row.payload_json, "provider_route")
        if provider_route != EXPECTED_PROVIDER_ROUTE:
            invalid_checks.append("route.provider_route_not_x")
            continue
        provider_route_x_count += 1

        shape_ready, shape_check = await _event_shape_is_consistent(
            repository=repository,
            row=row,
        )
        if not shape_ready:
            invalid_checks.append(shape_check or "aggregate.internal_consistency")
            continue

        try:
            route = route_resolver.resolve(row)
        except UnsupportedOutboxEventTypeError:
            invalid_checks.append("route.unsupported")
            continue

        if route.queue_name == EXPECTED_QUEUE_NAME:
            route_queue_valid_count += 1
        else:
            invalid_checks.append("route.queue")
            continue

        if route.stage_name == EXPECTED_STAGE_NAME:
            route_stage_valid_count += 1
        else:
            invalid_checks.append("route.stage")
            continue

        message = build_redis_queued_message(row, route)
        if not validate_redis_thin_payload_shape(message.as_stream_fields()):
            invalid_checks.append("redis.thin_payload_shape")
            continue
        thin_payload_valid_count += 1

        report["pending_x_enrich_event_selected_bucket"] = "one"
        report["provider_route_x_bucket"] = _bucket_count(provider_route_x_count)
        report["route_queue_valid_bucket"] = _bucket_count(route_queue_valid_count)
        report["route_stage_valid_bucket"] = _bucket_count(route_stage_valid_count)
        report["thin_payload_shape_valid_bucket"] = _bucket_count(thin_payload_valid_count)
    ...
    already_published_count = await repository.count_published_x_enrich_events()
    if already_published_count > 0:
        _set_status(report, STATUS_ALREADY_PUBLISHED)
        return None, raw_values, True

    if invalid_checks:
        _set_status(report, STATUS_INVALID_ROUTE)
        for check in invalid_checks:
            _set_status(report, report["contract_status"], check)
        return None, raw_values, False

    _set_status(report, STATUS_NO_PENDING)
    return None, raw_values, False
    ...
    *,
    ...
    try:
        ...
        _set_status(
            report,
            STATUS_DB_WRITE_FAILED_AFTER_REDIS,
            "database.write_after_redis_publish",
        )
        return ScriptResult(exit_code=1, report=report)
...
async def generate_report_async(
    *,
    ...
    try:
        ...
        repository = ArtifactEnrichOutboxRepository(session)
        ...
        if target is None:
            return ScriptResult(
                exit_code=0 if selection_valid else 1,
                report=report,
            )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = generate_report(
        ...
        approvals=PublishApprovals(
            artifact_enrich_x_route_publish_smoke=(
                args.approved_artifact_enrich_x_route_publish_smoke
            ),
            redis_publish=args.approved_redis_publish,
            event_outbox_published_update=args.approved_event_outbox_published_update,
